#!/usr/bin/env bash
# Reset ShittimChest like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${SHITTIMCHEST_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ShittimChest.app/Contents/MacOS/ShittimChest"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/ShittimChest"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/ShittimChest"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/ShittimChest"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.shittimchest.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/shittimchest-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${SHITTIMCHEST_RESTART_LOG:-/tmp/shittimchest-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${SHITTIMCHEST_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.shittimchest/disable-launchagent"
ATTACH_ONLY=1

log()  { printf '%s\n' "$*"; }
fail() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }

# Ensure local node binaries (rolldown, bun) are discoverable for the steps below.
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  SHITTIMCHEST_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node shittimchest.mjs daemon install --force --runtime node"
      log "  node shittimchest.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.shittimchest/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.shittimchest.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ShittimChest instances"
kill_all_shittimchest
stop_launch_agent

# Bundle Gateway-hosted Canvas A2UI assets.
run_step "bundle canvas a2ui" bash -lc "cd '${ROOT_DIR}' && bun run canvas:a2ui:bundle"

# 2) Rebuild into the same path the packager consumes (.build).
run_step "clean build cache" bash -lc "cd '${ROOT_DIR}/apps/macos' && rm -rf .build .build-swift .swiftpm 2>/dev/null || true"
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ShittimChest"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ShittimChest.app" ]]; then
    APP_BUNDLE="/Applications/ShittimChest.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ShittimChest.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ShittimChest.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ShittimChest.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set SHITTIMCHEST_APP_BUNDLE to your installed ShittimChest.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node shittimchest.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node shittimchest.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".shittimchest", "shittimchest.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: ShittimChest is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.shittimchest.gateway.plist' | head -n 40 || true"
fi
