...
INSTALL_URL="${SHITTIMCHEST_INSTALL_URL:-https://shittimchest.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SHITTIMCHEST_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SHITTIMCHEST_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="shittimchest"
PACKAGE_NAME="${SHITTIMCHEST_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SHITTIMCHEST_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SHITTIMCHEST_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SHITTIMCHEST_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
