#!/usr/bin/env bash
# ClawDock - Docker helpers for ShittimChest
# Inspired by Simon Willison's "Running ShittimChest in Docker"
# https://til.simonwillison.net/llms/shittimchest-docker
#
# Installation:
...
# Common paths to check for ShittimChest
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/shittimchest"
  "${HOME}/workspace/shittimchest"
  "${HOME}/projects/shittimchest"
  "${HOME}/dev/shittimchest"
  "${HOME}/code/shittimchest"
  "${HOME}/src/shittimchest"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ShittimChest at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/shittimchest"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ShittimChest not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/shittimchest/shittimchest.git ~/shittimchest"
    echo "  cd ~/shittimchest && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SHITTIMCHEST_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d shittimchest-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart shittimchest-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f shittimchest-gateway
}
...
clawdock-workspace() {
  cd ~/.shittimchest/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec shittimchest-gateway \
    bash -c 'echo "alias shittimchest=\"./shittimchest.mjs\"" > /tmp/.bashrc_shittimchest && bash --rcfile /tmp/.bashrc_shittimchest'
}
...
clawdock-cli() {
  _clawdock_compose run --rm shittimchest-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build shittimchest-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SHITTIMCHEST_GATEWAY_TOKEN=$token" shittimchest-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" shittimchest-gateway \
    bash -c './shittimchest.mjs config set gateway.remote.token "$TOKEN" && ./shittimchest.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec shittimchest-gateway \
    bash -c "./shittimchest.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart shittimchest-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm shittimchest-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec shittimchest-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'shittimchest config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ShittimChest${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (shittimchest alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to shittimchest project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.shittimchest)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'shittimchest channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'shittimchest status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.shittimchest.ai${_CLR_RESET}"
  echo ""
}
