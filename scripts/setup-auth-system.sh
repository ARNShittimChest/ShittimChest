#!/bin/bash
# Setup ShittimChest Auth Management System
...
echo "=== ShittimChest Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'shittimchest-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ShittimChest message: Send warning via ShittimChest itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/shittimchest-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/shittimchest-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/shittimchest-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now shittimchest-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status shittimchest-auth-monitor.timer"
echo ""
echo "Quick commands:"
