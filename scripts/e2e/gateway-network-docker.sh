...
IMAGE_NAME="shittimchest-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="shittimchest-net-e2e-$$"
GW_NAME="shittimchest-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "SHITTIMCHEST_GATEWAY_TOKEN=$TOKEN" \
  -e "SHITTIMCHEST_SKIP_CHANNELS=1" \
  -e "SHITTIMCHEST_SKIP_GMAIL_WATCHER=1" \
  -e "SHITTIMCHEST_SKIP_CRON=1" \
  -e "SHITTIMCHEST_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
