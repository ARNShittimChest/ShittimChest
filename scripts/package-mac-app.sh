...
# Build and bundle ShittimChest into a minimal .app we can open.
# Outputs to dist/ShittimChest.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ShittimChest.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ShittimChest"
BUNDLE_ID="${BUNDLE_ID:-ai.shittimchest.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/shittimchest/shittimchest/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ShittimChest/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ShittimChestBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ShittimChestGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ShittimChest"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ShittimChest"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ShittimChest"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ShittimChest" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ShittimChest/Resources/ShittimChest.icns" "$APP_ROOT/Contents/Resources/ShittimChest.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ShittimChest/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ShittimChestKit resources"
SHITTIMCHESTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ShittimChestKit_ShittimChestKit.bundle"
if [ -d "$SHITTIMCHESTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ShittimChestKit_ShittimChestKit.bundle"
  cp -R "$SHITTIMCHESTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ShittimChestKit_ShittimChestKit.bundle"
else
  echo "WARN: ShittimChestKit resource bundle not found at $SHITTIMCHESTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ShittimChest"
killall -q ShittimChest 2>/dev/null || true
