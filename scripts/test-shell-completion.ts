 ...
 * `shittimchest update`. Use it to verify the completion installation flow
...
\`shittimchest update\`, \`shittimchest doctor\`, and \`shittimchest onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ShittimChest Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`shittimchest completion --install\` later to enable.`));
    return;
  }
