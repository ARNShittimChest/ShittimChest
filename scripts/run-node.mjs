...
const shouldBuild = (deps) => {
  if (deps.env.SHITTIMCHEST_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SHITTIMCHEST_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[shittimchest] ${message}\n`);
};

const runShittimChest = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["shittimchest.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runShittimChest(deps);
  }
