...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core ShittimChest release numbers.\n\n`;
...
function stripWorkspaceShittimchestDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.shittimchest !== "workspace:*") {
    return false;
  }
  delete devDeps.shittimchest;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceShittimchestDevDependency(pkg);
