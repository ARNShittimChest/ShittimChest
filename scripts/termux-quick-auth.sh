...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/shittimchest/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ShittimChest Re-Auth" -c "After getting key, run: ssh $SERVER '~/shittimchest/scripts/mobile-reauth.sh'" --id shittimchest-auth
        ;;
    *)
