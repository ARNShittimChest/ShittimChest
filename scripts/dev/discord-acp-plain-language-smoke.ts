...
function resolveStateDir(): string {
  ...
  const home = process.env.SHITTIMCHEST_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".shittimchest");
}
...
function usage(): string {
  return (
    ...
    "1) ShittimChest spawned an ACP thread binding\n" +
    ...
    "  SHITTIMCHEST_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_DRIVER\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_AGENT\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_POLL_MS\n" +
    "  SHITTIMCHEST_DISCORD_SMOKE_THREAD_BINDINGS_PATH"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.SHITTIMCHEST_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.SHITTIMCHEST_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.SHITTIMCHEST_DISCORD_SMOKE_DRIVER_TOKEN ||
  ...
  const botTokenPrefix =
    ...
    process.env.SHITTIMCHEST_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.SHITTIMCHEST_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.SHITTIMCHEST_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.SHITTIMCHEST_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.SHITTIMCHEST_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.SHITTIMCHEST_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.SHITTIMCHEST_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `shittimchest-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from ShittimChest.`,
        diagnostics: {
          bindingCandidates: [
            {
