...
BASE_IMAGE="${BASE_IMAGE:-shittimchest-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-shittimchest-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=shittimchest.sandbox=1)
NOTE
