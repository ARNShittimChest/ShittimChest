...
If you believe you've found a security issue in ShittimChest, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [shittimchest/shittimchest](https://github.com/shittimchest/shittimchest)
- **macOS desktop app** — [shittimchest/shittimchest](https://github.com/shittimchest/shittimchest) (apps/macos)
- **iOS app** — [shittimchest/shittimchest](https://github.com/shittimchest/shittimchest) (apps/ios)
- **Android app** — [shittimchest/shittimchest](https://github.com/shittimchest/shittimchest) (apps/android)
- **ClawHub** — [shittimchest/clawhub](https://github.com/shittimchest/clawhub)
- **Trust and threat model** — [shittimchest/trust](https://github.com/shittimchest/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@shittimchest.ai](mailto:security@shittimchest.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.shittimchest.ai).
...
- Tested version details (ShittimChest version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to ShittimChest's documented trust boundaries.
- For exposed-secret reports: proof the credential is ShittimChest-owned (or grants access to ShittimChest-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at ShittimChest. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

ShittimChest is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
ShittimChest does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- ShittimChest can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need ShittimChest, use one VPS (or host/OS user boundary) per user.
...
- This is expected in ShittimChest's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of ShittimChest's trusted computing base for a gateway.
...
- Using ShittimChest in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.shittimchest`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not ShittimChest-owned and not granting access to ShittimChest-operated infrastructure/services) without demonstrated ShittimChest impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

ShittimChest security guidance assumes:

- The host where ShittimChest runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.shittimchest` state/config (including `shittimchest.json`) is effectively a trusted operator.
...
ShittimChest's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
ShittimChest separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the ShittimChest process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

ShittimChest uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/shittimchest` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/shittimchest` (or `shittimchest-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the ShittimChest-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use ShittimChest temp helpers (`resolvePreferredShittimChestTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-shittimchest-dir.ts`
...
For threat model + hardening guidance (including `shittimchest security audit --deep` and `--fix`), see:

- `https://docs.shittimchest.ai/gateway/security`
...
ShittimChest's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `shittimchest gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - ShittimChest keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `shittimchest security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__shittimchest__/canvas/`, `/__shittimchest__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__shittimchest__/canvas/`, `/__shittimchest__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

ShittimChest requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running ShittimChest in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v shittimchest-data:/app/data \
  shittimchest/shittimchest:latest
```
