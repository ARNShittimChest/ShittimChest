...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ShittimChestProtocol.AnyCodable(["ok": ShittimChestProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ShittimChestProtocol.AnyCodable(["ok": ShittimChestProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ShittimChestProtocol.AnyCodable([
            "runId": ShittimChestProtocol.AnyCodable("run-1"),
            "sessionKey": ShittimChestProtocol.AnyCodable("main"),
            "state": ShittimChestProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ShittimChestProtocol.AnyCodable(["a": ShittimChestProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
