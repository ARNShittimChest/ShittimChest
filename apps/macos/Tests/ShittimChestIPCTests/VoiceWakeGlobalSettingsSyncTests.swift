...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = ShittimChestProtocol.AnyCodable(["triggers": ["shittimchest", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["shittimchest", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = ShittimChestProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
