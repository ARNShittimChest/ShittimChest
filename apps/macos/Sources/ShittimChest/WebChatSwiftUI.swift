...
private let webChatSwiftLogger = Logger(subsystem: "ai.shittimchest", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ShittimChestChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ShittimChestChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ShittimChestChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ShittimChestChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ShittimChestGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ShittimChestChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ShittimChestAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ShittimChestChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ShittimChestChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ShittimChestChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ShittimChestChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "ShittimChest Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ShittimChestChatView>) -> NSViewController
    {
