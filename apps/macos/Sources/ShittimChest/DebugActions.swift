...
enum DebugActions {
    private static let verboseDefaultsKey = "shittimchest.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "shittimchest.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ShittimChestPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ShittimChest", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ShittimChestPaths.configURL
        guard
