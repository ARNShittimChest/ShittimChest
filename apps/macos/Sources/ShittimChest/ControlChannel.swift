...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ShittimChestProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.shittimchest", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ShittimChestKit.AnyCodable]()) {
                $0[$1.key] = ShittimChestKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ShittimChestProtocol.AnyCodable?) -> [String: ShittimChestProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ShittimChestProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ShittimChestKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ShittimChestProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("shittimchest.control.heartbeat")
    static let controlAgentEvent = Notification.Name("shittimchest.control.agent")
}
