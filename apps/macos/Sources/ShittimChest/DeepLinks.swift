...
private let deepLinkLogger = Logger(subsystem: "ai.shittimchest", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "ShittimChest is paused", message: "Unpause ShittimChest to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run ShittimChest agent?", message: body) else { return }
        }
