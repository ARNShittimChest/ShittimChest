...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.shittimchest.mac`) even if the shipped
    ...
    {
        if environment["SHITTIMCHEST_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "shittimchest.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "shittimchest.nixMode") { return true }
