...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.shittimchest", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ShittimChestSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ShittimChestSessionPreviewEntry,
    ...
    private static func cache(payload: ShittimChestSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ShittimChestChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ShittimChestKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ShittimChestKit.AnyCodable]) -> [ShittimChestChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ShittimChestChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ShittimChestChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ShittimChestChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ShittimChestChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ShittimChestChatMessage) -> String? {
