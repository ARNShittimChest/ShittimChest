...
enum CommandResolver {
    private static let projectRootDefaultsKey = "shittimchest.gatewayProjectRootPath"
    private static let helperName = "shittimchest"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let shittimchestEntry = root.appendingPathComponent("shittimchest.mjs").path
        if FileManager().isReadableFile(atPath: shittimchestEntry) { return shittimchestEntry }
        let binEntry = root.appendingPathComponent("bin/shittimchest.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__SHITTIMCHEST_ERR__' >&2
        \(message)
        __SHITTIMCHEST_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/shittimchest")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let shittimchestPaths = self.shittimchestManagedPaths(home: home)
        if !shittimchestPaths.isEmpty {
            extras.insert(contentsOf: shittimchestPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + shittimchestPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func shittimchestManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".shittimchest"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func shittimchestExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectShittimChestExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("shittimchest.mjs").path,
            root.appendingPathComponent("bin/shittimchest.js").path,
        ]
    ...
    static func hasAnyShittimChestInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.shittimchestExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func shittimchestNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let shittimchestPath = self.projectShittimChestExecutable(projectRoot: root) {
            return [shittimchestPath, subcommand] + extraArgs
        }
        if let shittimchestPath = self.shittimchestExecutable(searchPaths: searchPaths) {
            return [shittimchestPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "shittimchest", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            shittimchest entrypoint missing (looked for dist/index.js or shittimchest.mjs); run pnpm build.
            """
    ...
    static func shittimchestCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real shittimchest CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/shittimchest"
        ...
        \(cliSection)
        \(projectSection)
        if command -v shittimchest >/dev/null 2>&1; then
          CLI="$(command -v shittimchest)"
          shittimchest \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/shittimchest.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/shittimchest.mjs"
            node "$PRJ/shittimchest.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/shittimchest.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/shittimchest.js"
            node "$PRJ/bin/shittimchest.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent shittimchest"
          pnpm --silent shittimchest \(quotedArgs);
        else
          echo "shittimchest CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ShittimChestConfigFile.loadDict()
