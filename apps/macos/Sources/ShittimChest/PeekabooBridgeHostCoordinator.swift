...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.shittimchest", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ShittimChestPeekabooBridgeServices?
    private static var shittimchestSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("ShittimChest", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ShittimChestPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.shittimchestSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.shittimchestSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ShittimChestPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.shittimchest.peekaboo")
