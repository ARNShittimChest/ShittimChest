...
enum ShittimChestLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ShittimChestOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ShittimChestFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ShittimChestLogging.bootstrapIfNeeded()
        let label = ShittimChestLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ShittimChestOSLogHandler: LogHandler {
...
struct ShittimChestFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ShittimChestLogging.parseLabel(self.label)
