...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SHITTIMCHEST_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/shittimchest")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("shittimchest-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("shittimchest-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/shittimchest/ (rolling or stdout), or nil if none exist.
