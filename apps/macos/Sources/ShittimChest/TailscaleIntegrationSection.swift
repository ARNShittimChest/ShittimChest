...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.shittimchest/shittimchest.json. Prefer SHITTIMCHEST_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.shittimchest/shittimchest.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.shittimchest/shittimchest.json. Restart the gateway to apply."
        }
