...
@MainActor
final class ShittimChestAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.shittimchest.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.shittimchest.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.shittimchest.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    ...
    static let sessionKeyKey = "shittimchest.watch.sessionKey"
    static let actionPrimaryIDKey = "shittimchest.watch.action.primary.id"
    static let actionPrimaryLabelKey = "shittimchest.watch.action.primary.label"
    static let actionSecondaryIDKey = "shittimchest.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "shittimchest.watch.action.secondary.label"
    ...
    static let actionIDKeyPrefix = "shittimchest.watch.action.id."
    static let actionLabelKeyPrefix = "shittimchest.watch.action.label."
    static let categoryPrefix = "shittimchest.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ShittimChestWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ShittimChestWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ShittimChestWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "ShittimChest" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ShittimChestWatchAction]) -> [UNNotificationAction] {
...
extension ShittimChestApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("ShittimChest: installing uncaught exception handler")
