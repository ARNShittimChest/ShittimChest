...
final class MotionService: MotionServicing {
    func activities(params: ShittimChestMotionActivityParams) async throws -> ShittimChestMotionActivityPayload {
        ...
        let mapped: [ShittimChestMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        ShittimChestMotionActivityEntry(
        ...
        return ShittimChestMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ShittimChestPedometerParams) async throws -> ShittimChestPedometerPayload {
        ...
        let payload: ShittimChestPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ShittimChestPedometerPayload(
