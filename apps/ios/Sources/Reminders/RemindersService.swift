...
final class RemindersService: RemindersServicing {
    func list(params: ShittimChestRemindersListParams) async throws -> ShittimChestRemindersListPayload {
        ...
        let payload: [ShittimChestReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return ShittimChestReminderPayload(
        ...
        return ShittimChestRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ShittimChestRemindersAddParams) async throws -> ShittimChestRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ShittimChestReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ShittimChestRemindersAddPayload(reminder: payload)
    }
