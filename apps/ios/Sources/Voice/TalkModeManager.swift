...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SHITTIMCHEST_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ShittimChestTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.shittimchest", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ShittimChestTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ShittimChestTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ShittimChestTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ShittimChestTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ShittimChestTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ShittimChestTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ShittimChestTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ShittimChestTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ShittimChestTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: ShittimChestAgentEventPayload.self
            ) else {
                continue
            }
