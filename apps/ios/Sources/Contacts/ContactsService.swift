...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ShittimChestContactsSearchParams) async throws -> ShittimChestContactsSearchPayload {
        ...
        return ShittimChestContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ShittimChestContactsAddParams) async throws -> ShittimChestContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ShittimChestContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ShittimChestContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ShittimChestContactPayload {
        ShittimChestContactPayload(
