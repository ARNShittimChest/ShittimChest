...
struct IOSGatewayChatTransport: ShittimChestChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.shittimchest", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> ShittimChestChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ShittimChestChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ShittimChestChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ShittimChestChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ShittimChestChatAttachmentPayload]) async throws -> ShittimChestChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ShittimChestChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ShittimChestChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ShittimChestGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ShittimChestGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ShittimChestChatEventPayload.self)
                        {
