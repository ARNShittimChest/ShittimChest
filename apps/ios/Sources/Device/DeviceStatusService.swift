...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ShittimChestDeviceStatusPayload {
        ...
        return ShittimChestDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> ShittimChestDeviceInfoPayload {
        ...
        return ShittimChestDeviceInfoPayload(
    ...
    private func batteryStatus() -> ShittimChestBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ShittimChestBatteryState = switch device.batteryState {
        ...
        return ShittimChestBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ShittimChestThermalStatusPayload {
        let state: ShittimChestThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ShittimChestThermalStatusPayload(state: state)
    }

    private func storageStatus() -> ShittimChestStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return ShittimChestStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
