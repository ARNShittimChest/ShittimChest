...
private const val NOTIFICATION_CHANNEL_BASE_ID = "shittimchest.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "ShittimChest Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "ShittimChest Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "ShittimChest Active")
      }
