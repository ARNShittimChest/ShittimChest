...
import Foundation
import Testing
@testable import ShittimChestChatUI
...
private final class TestChatTransport: @unchecked Sendable, ShittimChestChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ShittimChestChatHistoryPayload]
    private let sessionsResponses: [ShittimChestChatSessionsListResponse]

    private let stream: AsyncStream<ShittimChestChatTransportEvent>
    private let continuation: AsyncStream<ShittimChestChatTransportEvent>.Continuation

    init(
        historyResponses: [ShittimChestChatHistoryPayload],
        sessionsResponses: [ShittimChestChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ShittimChestChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ShittimChestChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ShittimChestChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ShittimChestChatAttachmentPayload]) async throws -> ShittimChestChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ShittimChestChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ShittimChestChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ShittimChestChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ShittimChestChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ShittimChestChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ShittimChestChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ShittimChestChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ShittimChestChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ShittimChestChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ShittimChestChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ShittimChestChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
