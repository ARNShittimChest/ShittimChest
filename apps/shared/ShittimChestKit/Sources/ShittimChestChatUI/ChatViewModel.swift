...
private let chatUILogger = Logger(subsystem: "ai.shittimchest", category: "ShittimChestChatUI")

@MainActor
@Observable
public final class ShittimChestChatViewModel {
    public private(set) var messages: [ShittimChestChatMessage] = []
    ...
    public var attachments: [ShittimChestPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ShittimChestChatPendingToolCall] = []
    public private(set) var sessions: [ShittimChestChatSessionEntry] = []
    private let transport: any ShittimChestChatTransport
    ...
    private var pendingToolCallsById: [String: ShittimChestChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ShittimChestChatTransport) {
    ...
    public var sessionChoices: [ShittimChestChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [ShittimChestChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ShittimChestPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ShittimChestChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ShittimChestChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ShittimChestChatMessage) -> ShittimChestChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ShittimChestChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ShittimChestChatMessageContent(
                type: content.type,
                text: cleaned,
        ...
        return ShittimChestChatMessage(
            id: message.id,
            role: message.role,
    ...
    private static func messageIdentityKey(for message: ShittimChestChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ShittimChestChatMessage],
        incoming: [ShittimChestChatMessage]) -> [ShittimChestChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ShittimChestChatMessage]) -> [ShittimChestChatMessage] {
        var result: [ShittimChestChatMessage] = []
    ...
    private static func dedupeKey(for message: ShittimChestChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ShittimChestChatMessageContent] = [
            ShittimChestChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ShittimChestChatAttachmentPayload in
            ShittimChestChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ShittimChestChatSessionEntry {
        ShittimChestChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ShittimChestChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ShittimChestChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ShittimChestAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ShittimChestChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ShittimChestPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
