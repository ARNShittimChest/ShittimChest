...
struct ChatMessageBubble: View {
    let message: ShittimChestChatMessage
    let style: ShittimChestChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ShittimChestChatTheme.userText : ShittimChestChatTheme.assistantText
    ...
    private var inlineAttachments: [ShittimChestChatMessageContent] {
    ...
    private var toolCalls: [ShittimChestChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ShittimChestChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ShittimChestChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ShittimChestChatTheme.onboardingAssistantBubble
        }
        return ShittimChestChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ShittimChestChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ShittimChestChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ShittimChestChatTheme.userText : ShittimChestChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ShittimChestChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ShittimChestChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ShittimChestChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ShittimChestChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ShittimChestChatTheme.assistantText)
            }
        }
    }
}
