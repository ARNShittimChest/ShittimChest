...
@MainActor
struct ShittimChestChatComposer: View {
    @Bindable var viewModel: ShittimChestChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ShittimChestChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ShittimChestChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ShittimChestPendingAttachment.id)
                { (att: ShittimChestPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ShittimChestPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(ShittimChestChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ShittimChestChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ShittimChestChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ShittimChestChatTheme.subtleCard)
        .clipShape(Capsule())
    }
