...
type EnvSnapshot = {
  ...
  shittimchestHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    shittimchestHome: process.env.SHITTIMCHEST_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SHITTIMCHEST_HOME", snapshot.shittimchestHome);
  restoreKey("SHITTIMCHEST_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SHITTIMCHEST_HOME.
  delete process.env.SHITTIMCHEST_HOME;
  process.env.SHITTIMCHEST_STATE_DIR = path.join(base, ".shittimchest");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "shittimchest-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".shittimchest", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
