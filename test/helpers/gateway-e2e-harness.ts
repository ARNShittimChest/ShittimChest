...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `shittimchest-e2e-${name}-`));
  const configDir = path.join(homeDir, ".shittimchest");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SHITTIMCHEST_SKIP_PROVIDERS: "1",
          SHITTIMCHEST_SKIP_GMAIL_WATCHER: "1",
          SHITTIMCHEST_SKIP_CRON: "1",
          SHITTIMCHEST_SKIP_BROWSER_CONTROL_SERVER: "1",
          SHITTIMCHEST_SKIP_CANVAS_HOST: "1",
          SHITTIMCHEST_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
