---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ShittimChest deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ShittimChest cron scheduling for periodic checks, or version status checks on a machine running ShittimChest (laptop, workstation, Pi, VPS).
---

# ShittimChest Host Hardening

## Overview

Assess and harden the host running ShittimChest, then align it to a user-defined risk tolerance without breaking access. Use ShittimChest security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ShittimChest changes the host firewall, SSH, or OS updates; it does not.
...
5. ShittimChest gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ShittimChest can access sensitive data.
...
### 2) Run ShittimChest security audits (read-only)

As part of the default read-only checks, run `shittimchest security audit --deep`. Only offer alternatives if the user requests them:

1. `shittimchest security audit` (faster, non-probing)
2. `shittimchest security audit --json` (structured output)

Offer to apply ShittimChest safe defaults (numbered):

1. `shittimchest security audit --fix`

Be explicit that `--fix` only tightens ShittimChest defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ShittimChest version/update status (read-only)

As part of the default read-only checks, run `shittimchest update status`.
...
- Credential hygiene notes (location of ShittimChest creds, prefer disk encryption)
...
- ShittimChest security audit (re-run)
...
After ShittimChest install or first hardening pass, run at least one baseline audit and version check:

- `shittimchest security audit`
- `shittimchest security audit --deep`
- `shittimchest update status`

Ongoing monitoring is recommended. Use the ShittimChest cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `shittimchest cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `shittimchest update status`
...
Before creating, `shittimchest cron list` and match on exact `name`. If found, `shittimchest cron edit <id> ...`.
If not found, `shittimchest cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `shittimchest update status` (preferred for source checkouts and channels)
2. `npm view shittimchest version` (published npm version)

## ShittimChest command accuracy

Use only supported commands and flags:

- `shittimchest security audit [--deep] [--fix] [--json]`
- `shittimchest status` / `shittimchest status --deep`
- `shittimchest health --json`
- `shittimchest update status`
- `shittimchest cron add|list|runs|run`

Do not invent CLI flags or imply ShittimChest enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ShittimChest compaction:
