# ShittimChest ACP Bridge

This document describes how the ShittimChest ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`shittimchest acp` exposes an ACP agent over stdio and forwards prompts to a running
ShittimChest Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `shittimchest acp` and speaks ACP messages over stdio.
...
shittimchest acp --reset-session
```
...
`shittimchest acp` resolves the Gateway URL and auth from CLI flags or config:
