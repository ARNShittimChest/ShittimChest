...
describe("resolveGatewayConnection", () => {
  ...
  it("uses SHITTIMCHEST_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ SHITTIMCHEST_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers SHITTIMCHEST_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ SHITTIMCHEST_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
