...
function summarizeGroupPolicy(cfg: ShittimChestConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: ShittimChestConfig): boolean {
...
function collectModels(cfg: ShittimChestConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: ShittimChestConfig): Set<string> {
  const baseCfg: ShittimChestConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: ShittimChestConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ShittimChestConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ShittimChestConfig): boolean {
...
function isBrowserEnabled(cfg: ShittimChestConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ShittimChestConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: ShittimChestConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: ShittimChestConfig): {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SHITTIMCHEST_STATE_DIR on a local-only volume and re-run "${formatCliCommand("shittimchest security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: ShittimChestConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SHITTIMCHEST_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const shittimchestGatewayToken =
    typeof env.SHITTIMCHEST_GATEWAY_TOKEN === "string" && env.SHITTIMCHEST_GATEWAY_TOKEN.trim()
      ? env.SHITTIMCHEST_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-shittimchest-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: ShittimChestConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: ShittimChestConfig): SecurityAuditFinding[] {
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: shittimchest-sandbox-browser), " +
...
export function collectModelHygieneFindings(cfg: ShittimChestConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: ShittimChestConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: ShittimChestConfig): SecurityAuditFinding[] {
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "ShittimChest's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
