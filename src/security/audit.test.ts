...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: ShittimChestConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: ShittimChestConfig) => string[];
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { SHITTIMCHEST_STATE_DIR: channelSecurityStateDir },
      async () => await fn(channelSecurityStateDir),
    );
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.SHITTIMCHEST_GATEWAY_TOKEN;
    const prevPassword = process.env.SHITTIMCHEST_GATEWAY_PASSWORD;
    delete process.env.SHITTIMCHEST_GATEWAY_TOKEN;
    delete process.env.SHITTIMCHEST_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.SHITTIMCHEST_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.SHITTIMCHEST_GATEWAY_PASSWORD;
      } else {
        process.env.SHITTIMCHEST_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/shittimchest-safe-bins"];
    const cfg: ShittimChestConfig = {
      tools: {
        exec: {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "shittimchest.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      ...
      if (args[0] === "inspect" && args.at(-1) === "shittimchest-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "shittimchest-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("shittimchest-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("shittimchest-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "shittimchest-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "shittimchest-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-shittimchest.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): ShittimChestConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: ShittimChestConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.shittimchest/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: ShittimChestConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.SHITTIMCHEST_GATEWAY_TOKEN;
    process.env.SHITTIMCHEST_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ShittimChestConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<ShittimChestConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: ShittimChestConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.shittimchest",
      configPath: "/Users/test/Dropbox/.shittimchest/shittimchest.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: ShittimChestConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
    ...
    const cfg: ShittimChestConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@shittimchest/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@shittimchest/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@shittimchest/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@shittimchest/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        shittimchest: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        shittimchest: { extensions: ["../outside.js"] },
      }),
    );
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          shittimchest: { extensions: ["index.js"] },
        }),
      );
      await fs.writeFile(path.join(pluginDir, "index.js"), "export {};");
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.SHITTIMCHEST_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.SHITTIMCHEST_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
