...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `shittimchest-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      SHITTIMCHEST_LAUNCHD_LABEL: `ai.shittimchest.launchd-int-${testId}`,
      SHITTIMCHEST_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
