...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SHITTIMCHEST_GATEWAY_PORT).toBe("18789");
    expect(env.SHITTIMCHEST_GATEWAY_TOKEN).toBe("secret");
    expect(env.SHITTIMCHEST_SERVICE_MARKER).toBe("shittimchest");
    expect(env.SHITTIMCHEST_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SHITTIMCHEST_SERVICE_VERSION).toBe("string");
    expect(env.SHITTIMCHEST_SYSTEMD_UNIT).toBe("shittimchest-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SHITTIMCHEST_LAUNCHD_LABEL).toBe("ai.shittimchest.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SHITTIMCHEST_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SHITTIMCHEST_SYSTEMD_UNIT).toBe("shittimchest-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SHITTIMCHEST_LAUNCHD_LABEL).toBe("ai.shittimchest.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".shittimchest"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SHITTIMCHEST_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".shittimchest-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SHITTIMCHEST_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".shittimchest"));
  });

  it("uses SHITTIMCHEST_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SHITTIMCHEST_STATE_DIR: "/var/lib/shittimchest" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/shittimchest"));
  });

  it("expands ~ in SHITTIMCHEST_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SHITTIMCHEST_STATE_DIR: "~/shittimchest-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/shittimchest-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SHITTIMCHEST_STATE_DIR: "C:\\State\\shittimchest" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\shittimchest");
  });
});
