...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.SHITTIMCHEST_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service SHITTIMCHEST_GATEWAY_TOKEN does not match gateway.auth.token in shittimchest.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `shittimchest gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
