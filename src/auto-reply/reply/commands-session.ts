...
import { scheduleGatewaySigusr1Restart, triggerShittimChestRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ShittimChest in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerShittimChestRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ShittimChest via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
