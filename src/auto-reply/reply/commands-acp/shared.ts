...
export function resolveConfiguredAcpBackendId(cfg: ShittimChestConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: ShittimChestConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `shittimchest plugins install ${localPath}`;
    }
    return "shittimchest plugins install @shittimchest/acpx";
  }
