...
function listAgents(cfg: ShittimChestConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ShittimChestConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ShittimChestConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<ShittimChestConfig, EvaluatedBindingsCache>();
