...
describe("secret ref resolver", () => {
  ...
  it("resolves file refs in json mode", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-secrets-resolve-file-"));
  ...
  it("resolves exec refs with protocolVersion 1 response", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-secrets-resolve-exec-"));
    cleanupRoots.push(root);
  ...
  it("handles Homebrew-style symlinked exec commands with args only when explicitly allowed", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-secrets-resolve-homebrew-"));
    cleanupRoots.push(root);
  ...
  it("rejects exec refs when protocolVersion is not 1", async () => {
    ...
    const root = await fs.mkdtemp(
      path.join(os.tmpdir(), "shittimchest-secrets-resolve-exec-protocol-"),
    );
    cleanupRoots.push(root);
  ...
  it("rejects exec refs when response omits requested id", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-secrets-resolve-exec-id-"));
    cleanupRoots.push(root);
  ...
  it("times out file provider reads when timeoutMs elapses", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-secrets-resolve-timeout-"));
    cleanupRoots.push(root);
    const filePath = path.join(root, "secrets.json");
    await writeSecureFile(
      filePath,
