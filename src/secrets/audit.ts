...
import { resolveStateDir, type ShittimChestConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: ShittimChestConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.SHITTIMCHEST_AUTH_STORE_READONLY;
  process.env.SHITTIMCHEST_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as ShittimChestConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.SHITTIMCHEST_AUTH_STORE_READONLY;
    } else {
      process.env.SHITTIMCHEST_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
