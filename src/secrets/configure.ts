...
function getSecretProviders(config: ShittimChestConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: ShittimChestConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: ShittimChestConfig): ConfigureCandidate[] {
...
async function configureProvidersInteractive(config: ShittimChestConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in shittimchest.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "shittimchest secrets configure",
