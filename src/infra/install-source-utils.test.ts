...
const TEMP_DIR_PREFIX = "shittimchest-install-source-utils-";
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("shittimchest-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-shittimchest-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    const archivePath = path.join(cwd, "shittimchest-plugin-1.2.3.tgz");
    await fs.writeFile(archivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "shittimchest-plugin@1.2.3",
          name: "shittimchest-plugin",
          version: "1.2.3",
          filename: "shittimchest-plugin-1.2.3.tgz",
    ...
    const result = await runPack("shittimchest-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "shittimchest-plugin",
        version: "1.2.3",
        resolvedSpec: "shittimchest-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "shittimchest-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "shittimchest-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nshittimchest-plugin-1.2.3.tgz\n",
    });
  ...
  it("falls back to archive detected in cwd when npm pack stdout is empty", async () => {
    const cwd = await createTempDir("shittimchest-install-source-utils-");
    const archivePath = path.join(cwd, "shittimchest-plugin-1.2.3.tgz");
    await fs.writeFile(archivePath, "", "utf-8");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: " \n\n",
    ...
    const result = await packNpmSpecToArchive({
      spec: "shittimchest-plugin@1.2.3",
      timeoutMs: 5000,
      cwd,
    });
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr:
        "npm error code E404\nnpm error 404  '@shittimchest/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@shittimchest/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@shittimchest/whatsapp");
      expect(result.error).toContain("docs.shittimchest.ai/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("shittimchest-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "shittimchest-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@shittimchest/plugin-demo@2.0.0",
            filename: "shittimchest-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@shittimchest/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "shittimchest-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@shittimchest/plugin-demo@2.0.0",
      },
    });
  });
