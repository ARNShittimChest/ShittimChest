...
type EnsureShittimChestPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureShittimChestPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `shittimchest` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "shittimchest");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SHITTIMCHEST_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "shittimchest"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `shittimchest` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureShittimChestCliOnPath(opts: EnsureShittimChestPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SHITTIMCHEST_PATH_BOOTSTRAPPED)) {
    return;
  }
