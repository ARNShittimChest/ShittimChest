...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__shittimchest_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "shittimchest-root");
...
describe("resolveShittimChestPackageRoot", () => {
  let resolveShittimChestPackageRoot: typeof import("./shittimchest-root.js").resolveShittimChestPackageRoot;
  let resolveShittimChestPackageRootSync: typeof import("./shittimchest-root.js").resolveShittimChestPackageRootSync;

  beforeAll(async () => {
    ({ resolveShittimChestPackageRoot, resolveShittimChestPackageRootSync } =
      await import("./shittimchest-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "shittimchest");
    const pkgRoot = path.join(project, "node_modules", "shittimchest");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "shittimchest" }));

    expect(resolveShittimChestPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "shittimchest");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "shittimchest.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "shittimchest" }));

    expect(resolveShittimChestPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveShittimChestPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-shittimchest package roots", async () => {
    const pkgRoot = fx("not-shittimchest");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-shittimchest" }));

    expect(resolveShittimChestPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveShittimChestPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveShittimChestPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
