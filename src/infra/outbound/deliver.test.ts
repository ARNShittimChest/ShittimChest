...
import { resolvePreferredShittimChestTmpDir } from "../tmp-shittimchest-dir.js";
...
const telegramChunkConfig: ShittimChestConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: ShittimChestConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes ShittimChest tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredShittimChestTmpDir()]),
      }),
    );
  });

  it("includes ShittimChest tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes ShittimChest tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes ShittimChest tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: ShittimChestConfig = { channels: { signal: { mediaMaxMb: 2 } } };
