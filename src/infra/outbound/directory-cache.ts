...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ShittimChestConfig | null = null;
  ...
  get(key: string, cfg: ShittimChestConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ShittimChestConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: ShittimChestConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: ShittimChestConfig): void {
