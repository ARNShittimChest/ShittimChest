...
let ensureShittimChestCliOnPath: typeof import("./path-env.js").ensureShittimChestCliOnPath;

describe("ensureShittimChestCliOnPath", () => {
  const envKeys = [
    "PATH",
    "SHITTIMCHEST_PATH_BOOTSTRAPPED",
    "SHITTIMCHEST_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureShittimChestCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling shittimchest exists", () => {
    const tmp = abs("/tmp/shittimchest-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "shittimchest");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.SHITTIMCHEST_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.SHITTIMCHEST_PATH_BOOTSTRAPPED = "1";
    ensureShittimChestCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/shittimchest-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "shittimchest");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/shittimchest-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "shittimchest");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "shittimchest");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/shittimchest-path/case-linuxbrew");
