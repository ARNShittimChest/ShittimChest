...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __shittimchestDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__shittimchestDiagnosticEventsState) {
    globalStore.__shittimchestDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__shittimchestDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: ShittimChestConfig): boolean {
