...
import { triggerShittimChestRestart } from "./restart.js";
 ...
 * - SHITTIMCHEST_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.SHITTIMCHEST_NO_RESPAWN)) {
  ...
  if (isLikelySupervisedProcess(process.env)) {
    ...
    if (process.platform === "darwin" && process.env.SHITTIMCHEST_LAUNCHD_LABEL?.trim()) {
      const restart = triggerShittimChestRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
