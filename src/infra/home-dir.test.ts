...
describe("resolveEffectiveHomeDir", () => {
  it("prefers SHITTIMCHEST_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands SHITTIMCHEST_HOME when set to ~", () => {
    const env = {
      SHITTIMCHEST_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for SHITTIMCHEST_HOME", () => {
    const result = resolveRequiredHomeDir(
      { SHITTIMCHEST_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when SHITTIMCHEST_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ SHITTIMCHEST_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { SHITTIMCHEST_HOME: "/srv/shittimchest-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/shittimchest-home")}/x`);
  });
