...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("shittimchest-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("shittimchest-fs-safe-root-");
    const outside = await tempDirs.make("shittimchest-fs-safe-outside-");
...
describe("tilde expansion in file tools", () => {
  ...
  it("reads a file via ~/path after HOME override", async () => {
    const root = await tempDirs.make("shittimchest-tilde-test-");
    const originalHome = process.env.HOME;
  ...
  it("rejects ~/path that resolves outside root", async () => {
    const root = await tempDirs.make("shittimchest-tilde-outside-");
