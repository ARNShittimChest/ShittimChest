...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SHITTIMCHEST_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.SHITTIMCHEST_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveShittimChestPackageRoot } = await import("./shittimchest-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveShittimChestPackageRoot).mockResolvedValue("/opt/shittimchest");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/shittimchest",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/shittimchest/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        process.execPath,
        "/opt/shittimchest/dist/entry.js",
        "update",
        "--yes",
        "--channel",
        "beta",
