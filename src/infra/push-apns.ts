...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SHITTIMCHEST_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SHITTIMCHEST_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SHITTIMCHEST_APNS_TEAM_ID and SHITTIMCHEST_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SHITTIMCHEST_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SHITTIMCHEST_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SHITTIMCHEST_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SHITTIMCHEST_APNS_PRIVATE_KEY_P8 or SHITTIMCHEST_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SHITTIMCHEST_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createShittimChestPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    shittimchest: createShittimChestPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
