...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ShittimChestConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ShittimChestConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ShittimChestConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ShittimChestConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ShittimChestConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ShittimChestConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ShittimChestConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
