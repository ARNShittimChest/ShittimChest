...
const DEFAULT_PACKAGE_NAME = "shittimchest";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "shittimchest.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "shittimchest doctor entry",
    ...
    const doctorStep = await runStep(
      step("shittimchest doctor", doctorArgv, gitRoot, { SHITTIMCHEST_UPDATE_IN_PROGRESS: "1" }),
    );
