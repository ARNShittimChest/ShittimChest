...
vi.mock("./shittimchest-root.js", () => ({
  resolveShittimChestPackageRoot: vi.fn(async () => null),
  resolveShittimChestPackageRootSync: vi.fn(() => null),
}));
...
let shittimchestRoot: typeof import("./shittimchest-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    shittimchestRoot = await import("./shittimchest-root.js");
  });
  ...
  it("uses resolveShittimChestPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/shittimchest");
    (
      shittimchestRoot.resolveShittimChestPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/shittimchest"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "shittimchest" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "shittimchest.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-shittimchest");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/shittimchest-bundle");
    (
      shittimchestRoot.resolveShittimChestPackageRootSync as unknown as ReturnType<typeof vi.fn>
