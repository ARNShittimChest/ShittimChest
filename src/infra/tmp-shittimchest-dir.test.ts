...
function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `shittimchest-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_SHITTIMCHEST_TMP_DIR) {
  ...
  const resolved = resolvePreferredShittimChestTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredShittimChestTmpDir", () => {
  it("prefers /tmp/shittimchest when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SHITTIMCHEST_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/shittimchest when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SHITTIMCHEST_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/shittimchest when /tmp/shittimchest is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/shittimchest when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/shittimchest is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/shittimchest is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/shittimchest is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback ShittimChest temp dir/);
  });
