...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SHITTIMCHEST_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*shittimchest-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "shittimchest";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "shittimchest");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "shittimchest-gw");
  ...
  records.push(`_shittimchest-gw._tcp IN PTR ${instanceLabel}._shittimchest-gw._tcp`);
  records.push(`${instanceLabel}._shittimchest-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._shittimchest-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; shittimchest-content-hash: ${contentHash}\n${contentBody}`;
}
