...
export function triggerShittimChestRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SHITTIMCHEST_SYSTEMD_UNIT,
        process.env.SHITTIMCHEST_PROFILE,
      );
  ...
  const label =
    process.env.SHITTIMCHEST_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SHITTIMCHEST_PROFILE);
