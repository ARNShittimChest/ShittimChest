...
const TEMP_MEDIA_PREFIX = "shittimchest-media-";
...
function createGroqAudioConfig(): ShittimChestConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ShittimChestConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ShittimChestConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ShittimChestConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredShittimChestTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        SHITTIMCHEST_AGENT_DIR: isolatedAgentDir,
