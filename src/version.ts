...
declare const __SHITTIMCHEST_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "shittimchest";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["SHITTIMCHEST_VERSION"],
      env["SHITTIMCHEST_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current ShittimChest version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __SHITTIMCHEST_VERSION__ === "string" && __SHITTIMCHEST_VERSION__) ||
  process.env.SHITTIMCHEST_BUNDLED_VERSION ||
