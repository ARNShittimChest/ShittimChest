...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-exec-approvals-"));
    const previousShittimChestHome = process.env.SHITTIMCHEST_HOME;
    process.env.SHITTIMCHEST_HOME = tempHome;
    saveExecApprovals(params.approvals);
    try {
      return await params.run({ tempHome });
    } finally {
      if (previousShittimChestHome === undefined) {
        delete process.env.SHITTIMCHEST_HOME;
      } else {
        process.env.SHITTIMCHEST_HOME = previousShittimChestHome;
      }
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-approval-cwd-link-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-approval-cwd-parent-link-"));
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-approval-cwd-real-"));
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["shittimchest status; id", "shittimchest status; cat /etc/passwd"];
