...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SHITTIMCHEST_BUNDLED_HOOKS_DIR"]);
    process.env.SHITTIMCHEST_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
