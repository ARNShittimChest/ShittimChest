...
homepage: https://docs.shittimchest.ai/automation/hooks#command-logger
metadata:
  {
    "shittimchest":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.shittimchest/logs/commands.log`
...
`~/.shittimchest/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.shittimchest/logs/commands.log ~/.shittimchest/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/shittimchest`:
   ```
   /home/username/.shittimchest/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
