...
This directory contains hooks that ship with ShittimChest. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.shittimchest/shittimchest.json`:
...
2. Restart gateway: `pkill -9 -f 'shittimchest.*gateway' && pnpm shittimchest gateway`
3. Enable the hook: `shittimchest hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.shittimchest.ai/automation/hooks
