...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => ShittimChestConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("shittimchest-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): ShittimChestConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
