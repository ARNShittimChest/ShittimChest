...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@shittimchest/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@shittimchest/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@shittimchest/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@shittimchest/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @shittimchest/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      mapNpmResolutionMetadata({
        name: "@shittimchest/plugin-alpha",
        version: "1.2.3",
        resolvedSpec: "@shittimchest/plugin-alpha@1.2.3",
      ...
      resolvedName: "@shittimchest/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@shittimchest/plugin-alpha@1.2.3",
        installPath: "/tmp/shittimchest/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@shittimchest/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @shittimchest/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@shittimchest/plugin-alpha@latest",
      true,
      "/tmp/shittimchest/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@shittimchest/plugin-alpha@latest",
      installPath: "/tmp/shittimchest/extensions/alpha",
      version: "1.2.3",
