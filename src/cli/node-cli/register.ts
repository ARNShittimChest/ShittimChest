    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "shittimchest node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["shittimchest node status", "Check node host service status."],
          ["shittimchest node install", "Install the node host service."],
          ["shittimchest node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.shittimchest.ai/cli/node")}\n`,
    );
