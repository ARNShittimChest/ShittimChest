...
import type { ShittimChestConfig, ConfigFileSnapshot } from "../config/types.shittimchest.js";
...
vi.mock("../infra/shittimchest-root.js", () => ({
  resolveShittimChestPackageRoot: vi.fn(),
}));
...
const { resolveShittimChestPackageRoot } = await import("../infra/shittimchest-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/shittimchest-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as ShittimChestConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/shittimchest-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveShittimChestPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("shittimchest-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveShittimChestPackageRoot).mockClear();
    ...
    vi.mocked(resolveShittimChestPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("shittimchest");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/shittimchest-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "shittimchest-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("ShittimChest update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ShittimChestConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveShittimChestPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("shittimchest-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SHITTIMCHEST_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SHITTIMCHEST_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("shittimchest-update-wizard");
    await withEnvAsync({ SHITTIMCHEST_GIT_DIR: tempDir }, async () => {
      setTty(true);
