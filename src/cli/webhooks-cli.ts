...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.shittimchest.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + ShittimChest hooks")
    ...
    .option("--hook-url <url>", "ShittimChest hook URL")
    .option("--hook-token <token>", "ShittimChest hook token")
