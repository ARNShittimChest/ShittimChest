...
const EXAMPLES = [
  ["shittimchest models --help", "Show detailed help for the models command."],
  [
    "shittimchest channels login --verbose",
    "Link personal WhatsApp Web and show QR + connection logs.",
  ],
  [
    'shittimchest message send --target +15555550123 --message "Hi" --json',
  ...
  [
    'shittimchest message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.shittimchest-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates SHITTIMCHEST_STATE_DIR/SHITTIMCHEST_CONFIG_PATH under ~/.shittimchest-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.shittimchest.ai/cli");
