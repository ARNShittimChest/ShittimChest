...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "shittimchest", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "shittimchest", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "shittimchest", "status"]);
  });
