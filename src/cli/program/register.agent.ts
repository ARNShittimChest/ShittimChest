...
${formatHelpExamples([
  ['shittimchest agent --to +15555550123 --message "status update"', "Start a new session."],
  ['shittimchest agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'shittimchest agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'shittimchest agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['shittimchest agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'shittimchest agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.shittimchest.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.shittimchest.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "shittimchest agents set-identity --identity-file ~/.shittimchest/workspace/IDENTITY.md --agent main",
