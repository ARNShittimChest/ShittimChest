...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-bundled-skills-test-"));
    process.env.SHITTIMCHEST_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
