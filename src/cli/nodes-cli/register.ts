    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["shittimchest nodes status", "List known nodes with live status."],
          ["shittimchest nodes pairing pending", "Show pending node pairing requests."],
          ['shittimchest nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["shittimchest nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.shittimchest.ai/cli/nodes")}\n`,
    );
