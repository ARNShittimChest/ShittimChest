...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.SHITTIMCHEST_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.SHITTIMCHEST_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.SHITTIMCHEST_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.SHITTIMCHEST_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.SHITTIMCHEST_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.SHITTIMCHEST_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `shittimchest-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `shittimchest-restart-${timestamp}.bat`;
