...
import { resolveShittimChestPackageRoot } from "../../infra/shittimchest-root.js";
...
const SHITTIMCHEST_REPO_URL = "https://github.com/shittimchest/shittimchest.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "shittimchest";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("shittimchest@")) {
    return trimmed.slice("shittimchest@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.SHITTIMCHEST_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveShittimChestPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SHITTIMCHEST_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SHITTIMCHEST_GIT_DIR points at a non-git directory: ${params.dir}. Set SHITTIMCHEST_GIT_DIR to an empty folder or an shittimchest checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SHITTIMCHEST_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "shittimchest.mjs");
