...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update ShittimChest and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["shittimchest update", "Update a source checkout (git)"],
        ["shittimchest update --channel beta", "Switch to beta channel (git + npm)"],
        ["shittimchest update --channel dev", "Switch to dev channel (git + npm)"],
        ["shittimchest update --tag beta", "One-off update to a dist-tag or version"],
        ["shittimchest update --dry-run", "Preview actions without changing anything"],
        ["shittimchest update --no-restart", "Update without restarting the service"],
        ["shittimchest update --json", "Output result as JSON"],
        ["shittimchest update --yes", "Non-interactive (accept downgrade prompts)"],
        ["shittimchest update wizard", "Interactive update wizard"],
        ["shittimchest --update", "Shorthand for shittimchest update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run shittimchest update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.shittimchest.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["shittimchest update status", "Show channel + version status."],
          ["shittimchest update status --json", "JSON output."],
          ["shittimchest update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.shittimchest.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
