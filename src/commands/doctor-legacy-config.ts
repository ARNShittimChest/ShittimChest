...
export function normalizeCompatibilityConfigValues(cfg: ShittimChestConfig): {
  config: ShittimChestConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: ShittimChestConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as ShittimChestConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as ShittimChestConfig["browser"],
    };
    changes.push(
