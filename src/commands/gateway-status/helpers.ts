...
import type { ShittimChestConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.SHITTIMCHEST_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.SHITTIMCHEST_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: ShittimChestConfig) {
