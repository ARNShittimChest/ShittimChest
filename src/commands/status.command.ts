...
export async function statusCommand(
  ...
  runtime.log(theme.heading("ShittimChest status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`shittimchest devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("shittimchest security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("shittimchest security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.shittimchest.ai/faq");
  runtime.log("Troubleshooting: https://docs.shittimchest.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("shittimchest status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("shittimchest logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("shittimchest status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("shittimchest gateway probe")}`);
  }
}
