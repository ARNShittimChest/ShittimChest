...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ShittimChestConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ShittimChestConfig) => { changed: boolean; next: ShittimChestConfig };
  defaultModel: string;
  overrideConfig: ShittimChestConfig;
  alreadyDefaultConfig: ShittimChestConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ShittimChestConfig) => config,
      applyDefaultConfig: (config: ShittimChestConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ShittimChestConfig = { agents: { defaults: {} } };
