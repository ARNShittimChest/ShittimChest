...
export const resolveShittimChestPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadShittimChestPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/shittimchest.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveShittimChestPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SHITTIMCHEST_STATE_DIR;
  originalUpdateInProgress = process.env.SHITTIMCHEST_UPDATE_IN_PROGRESS;
  process.env.SHITTIMCHEST_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SHITTIMCHEST_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SHITTIMCHEST_UPDATE_IN_PROGRESS;
  } else {
    process.env.SHITTIMCHEST_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
