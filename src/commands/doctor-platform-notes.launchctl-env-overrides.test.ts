...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SHITTIMCHEST_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SHITTIMCHEST_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SHITTIMCHEST_GATEWAY_TOKEN");
    expect(message).not.toContain("SHITTIMCHEST_GATEWAY_PASSWORD");
  });
