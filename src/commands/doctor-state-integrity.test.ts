...
type EnvSnapshot = {
  HOME?: string;
  SHITTIMCHEST_HOME?: string;
  SHITTIMCHEST_STATE_DIR?: string;
  SHITTIMCHEST_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    SHITTIMCHEST_HOME: process.env.SHITTIMCHEST_HOME,
    SHITTIMCHEST_STATE_DIR: process.env.SHITTIMCHEST_STATE_DIR,
    SHITTIMCHEST_OAUTH_DIR: process.env.SHITTIMCHEST_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: ShittimChestConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: ShittimChestConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.SHITTIMCHEST_HOME = tempHome;
    process.env.SHITTIMCHEST_STATE_DIR = path.join(tempHome, ".shittimchest");
    delete process.env.SHITTIMCHEST_OAUTH_DIR;
    fs.mkdirSync(process.env.SHITTIMCHEST_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when SHITTIMCHEST_OAUTH_DIR is explicitly configured", async () => {
    process.env.SHITTIMCHEST_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ShittimChestConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints shittimchest-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: ShittimChestConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/shittimchest sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/shittimchest sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /shittimchest sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
