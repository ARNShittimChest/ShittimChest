...
function resolveGatewayAuthToken(
  ...
  const envToken = env.SHITTIMCHEST_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing ShittimChest gateway next.");
      }
    }
  }
