...
export async function loadValidConfigOrThrow(): Promise<ShittimChestConfig> {
...
export async function updateConfig(
  mutator: (cfg: ShittimChestConfig) => ShittimChestConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ShittimChestConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ShittimChestConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("shittimchest agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
