...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveShittimChestAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("shittimchest models auth setup-token")}\` or \`${formatCliCommand("shittimchest configure")}\`.`
          : `Run \`${formatCliCommand("shittimchest configure")}\` or set an API key env var.`;
