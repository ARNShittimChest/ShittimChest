...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveShittimChestAgentDir: vi.fn().mockReturnValue("/tmp/shittimchest-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/shittimchest-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/shittimchest-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveShittimChestAgentDir: mocks.resolveShittimChestAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/shittimchest-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveShittimChestAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/shittimchest-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/shittimchest-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/shittimchest-agent-custom");
