 ...
 * Runs as part of `shittimchest doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("shittimchest config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("shittimchest memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("shittimchest configure --section model")}`,
        `- To disable: ${formatCliCommand("shittimchest config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("shittimchest memory status --deep")}`,
      ].join("\n"),
      "Memory search",
