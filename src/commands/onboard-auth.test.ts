...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "SHITTIMCHEST_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "SHITTIMCHEST_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under SHITTIMCHEST_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("shittimchest-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-oauth-sync-"));
    ...
    process.env.SHITTIMCHEST_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-oauth-nosync-"));
    process.env.SHITTIMCHEST_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside SHITTIMCHEST_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-oauth-external-"));
    process.env.SHITTIMCHEST_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* SHITTIMCHEST_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SHITTIMCHEST_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("shittimchest-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
