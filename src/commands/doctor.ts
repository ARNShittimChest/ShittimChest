...
function resolveMode(cfg: ShittimChestConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("ShittimChest doctor");

  const root = await resolveShittimChestPackageRoot({
  ...
  let cfg: ShittimChestConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("shittimchest configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("shittimchest config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("shittimchest setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("shittimchest doctor --fix")}" to apply changes.`);
  }
