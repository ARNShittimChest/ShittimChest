...
import type { ShittimChestConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ShittimChestConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("shittimchest security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.shittimchest/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("shittimchest approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.shittimchest.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("shittimchest configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("shittimchest config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("shittimchest doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("shittimchest config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('shittimchest config set session.dmScope "per-channel-peer"') +
