...
type PromptDefaultModelResult = { model?: string; config?: ShittimChestConfig };
...
function resolveConfiguredModelRaw(cfg: ShittimChestConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: ShittimChestConfig): string[] {
...
export function applyPrimaryModel(cfg: ShittimChestConfig, model: string): ShittimChestConfig {
...
export function applyModelAllowlist(cfg: ShittimChestConfig, models: string[]): ShittimChestConfig {
  const defaults = cfg.agents?.defaults;
