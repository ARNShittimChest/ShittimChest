...
export function requireShittimChestAgentDir(): string {
  const agentDir = process.env.SHITTIMCHEST_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SHITTIMCHEST_AGENT_DIR not set");
  }
  return agentDir;
}
