  ...
  while (true) {
    ...
    if (source === "env") {
      ...
      await params.prompter.note(
        `Validated environment variable ${envVar}. ShittimChest will store a reference, not the key value.`,
        "Reference validated",
      );
    ...
    try {
      ...
      await params.prompter.note(
        `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. ShittimChest will store a reference, not the key value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    } catch (error) {
    ...
    options: [
      {
        ...
        hint: "Stores the key directly in ShittimChest config",
      },
      {
        value: "ref",
