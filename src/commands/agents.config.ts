...
function resolveAgentName(cfg: ShittimChestConfig, agentId: string) {
...
function resolveAgentModel(cfg: ShittimChestConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ShittimChestConfig): AgentSummary[] {
