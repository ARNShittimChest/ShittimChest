...
beforeAll(() => {
  envSnapshot = captureEnv(["SHITTIMCHEST_PROFILE"]);
  process.env.SHITTIMCHEST_PROFILE = "isolated";
});
...
vi.mock("../infra/shittimchest-root.js", () => ({
  resolveShittimChestPackageRoot: vi.fn().mockResolvedValue("/tmp/shittimchest"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/shittimchest/pnpm-lock.yaml",
      markerPath: "/tmp/shittimchest/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.shittimchest.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.shittimchest.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("ShittimChest status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("shittimchest status --all") ||
          l.includes("shittimchest --profile isolated status --all") ||
          l.includes("shittimchest status --all") ||
          l.includes("shittimchest --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SHITTIMCHEST_GATEWAY_TOKEN = "abcd1234";
    try {
