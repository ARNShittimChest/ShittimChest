...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.SHITTIMCHEST_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/shittimchest" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "shittimchest",
        profiles: {
          shittimchest: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchShittimChestChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.SHITTIMCHEST_GATEWAY_PORT;
  process.env.SHITTIMCHEST_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.SHITTIMCHEST_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.SHITTIMCHEST_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SHITTIMCHEST_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.SHITTIMCHEST_GATEWAY_PASSWORD;
  } else {
    process.env.SHITTIMCHEST_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
