import type { BrowserConfig, BrowserProfileConfig, ShittimChestConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SHITTIMCHEST_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "shittimchest" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SHITTIMCHEST_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "shittimchest" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ShittimChest browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for shittimchest/shittimchest CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ShittimChestConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SHITTIMCHEST_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "shittimchest";
