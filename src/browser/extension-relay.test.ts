...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "SHITTIMCHEST_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "SHITTIMCHEST_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.SHITTIMCHEST_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.SHITTIMCHEST_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.SHITTIMCHEST_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-shittimchest-relay-token");
    expect(headers["x-shittimchest-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${cdpUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-shittimchest-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.SHITTIMCHEST_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(cdpUrl)["x-shittimchest-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${cdpUrl}/json/version`, {
      headers: { "x-shittimchest-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-shittimchest-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "ShittimChest/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an shittimchest relay", async () => {
    const port = await getFreePort();
