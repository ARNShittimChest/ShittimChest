...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredShittimChestTmpDir: vi.fn(() => "/tmp/shittimchest"),
}));
vi.mock("../infra/tmp-shittimchest-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredShittimChestTmpDir.mockReturnValue("/tmp/shittimchest");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredShittimChestTmpDir.mockReturnValue("/tmp/shittimchest-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "shittimchest-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "shittimchest-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredShittimChestTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "shittimchest-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "shittimchest-preferred", "downloads")}${path.sep}`),
    );
  });
