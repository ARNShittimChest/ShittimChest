...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".shittimchest-profile-decorated");
}
 ...
 */
export function decorateShittimChestProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SHITTIMCHEST_BROWSER_COLOR).toUpperCase();
