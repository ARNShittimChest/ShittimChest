...
const RELAY_TOKEN_CONTEXT = "shittimchest-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const SHITTIMCHEST_RELAY_BROWSER = "ShittimChest/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.SHITTIMCHEST_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or SHITTIMCHEST_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedShittimChestRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === SHITTIMCHEST_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
