...
vi.mock("./chrome.js", () => ({
  resolveShittimChestUserDataDir: vi.fn(() => "/tmp/shittimchest-test/shittimchest/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveShittimChestUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "shittimchest-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveShittimChestUserDataDir).mockReturnValue(userDataDir);
