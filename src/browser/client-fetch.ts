...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-shittimchest-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-shittimchest-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-shittimchest-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the ShittimChest gateway (ShittimChest.app menubar, or \`${formatCliCommand("shittimchest gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the ShittimChest browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the ShittimChest browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
