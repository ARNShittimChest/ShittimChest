  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          shittimchest: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SHITTIMCHEST_GATEWAY_PORT;
    process.env.SHITTIMCHEST_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SHITTIMCHEST_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SHITTIMCHEST_GATEWAY_PASSWORD;
