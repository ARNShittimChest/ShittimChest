...
describe("Slack native command argument menus", () => {
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("shittimchest_cmdarg")).toBe(true);
    expect(testHarness.options.has("shittimchest_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("shittimchest_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("shittimchest_cmdarg");
    expect(blockId).toContain("shittimchest_cmdarg_ext:");
    const token = (blockId ?? "").slice("shittimchest_cmdarg_ext:".length);
