...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ SHITTIMCHEST_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.SHITTIMCHEST_BASE_ENV ?? "") + "|" + (process.env.SHITTIMCHEST_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { SHITTIMCHEST_TEST_ENV: "ok" },
        },
      );
