...
describe("shared/frontmatter", () => {
  ...
  test("resolveShittimChestManifestBlock parses JSON5 metadata and picks shittimchest block", () => {
    const frontmatter = {
      metadata: "{ shittimchest: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveShittimChestManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveShittimChestManifestBlock returns undefined for invalid input", () => {
    expect(resolveShittimChestManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveShittimChestManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveShittimChestManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
