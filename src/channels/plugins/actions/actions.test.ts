...
function telegramCfg(): ShittimChestConfig {
  return { channels: { telegram: { botToken: "tok" } } } as ShittimChestConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: ShittimChestConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as ShittimChestConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as ShittimChestConfig;
...
function createSignalAccountOverrideCfg(): ShittimChestConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as ShittimChestConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as ShittimChestConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as ShittimChestConfig,
        accountId: undefined,
        params: {
