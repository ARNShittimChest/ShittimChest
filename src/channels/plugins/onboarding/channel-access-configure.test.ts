...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: ShittimChestConfig, policy: ChannelAccessPolicy) => ShittimChestConfig;
  resolveAllowlist: (params: { cfg: ShittimChestConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: ShittimChestConfig; resolved: TResolved }) => ShittimChestConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: ShittimChestConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: ShittimChestConfig }) => params.cfg);
