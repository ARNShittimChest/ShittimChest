...
function setWhatsAppDmPolicy(cfg: ShittimChestConfig, dmPolicy: DmPolicy): ShittimChestConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ShittimChestConfig, allowFrom?: string[]): ShittimChestConfig {
...
async function detectWhatsAppLinked(cfg: ShittimChestConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so ShittimChest can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for ShittimChest" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("shittimchest channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
