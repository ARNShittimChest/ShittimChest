...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@shittimchest/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "shittimchest-catalog-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@shittimchest/demo-channel",
            shittimchest: {
              channel: {
                id: "demo-channel",
                label: "Demo Channel",
              ...
              install: {
                npmSpec: "@shittimchest/demo-channel",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: ShittimChestConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
