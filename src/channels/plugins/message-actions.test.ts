...
describe("message action capability checks", () => {
  ...
  it("aggregates buttons/card support across plugins", () => {
    ...
    expect(supportsChannelMessageButtons({} as ShittimChestConfig)).toBe(true);
    expect(supportsChannelMessageCards({} as ShittimChestConfig)).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(supportsChannelMessageCardsForChannel({ cfg: {} as ShittimChestConfig })).toBe(false);
  });
});
