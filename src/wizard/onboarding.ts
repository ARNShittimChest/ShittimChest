...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "ShittimChest is a hobby project and still in beta. Expect sharp edges.",
      "By default, ShittimChest is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "ShittimChest is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run ShittimChest.",
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Must read: https://docs.shittimchest.ai/gateway/security",
    ].join("\n"),
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("ShittimChest onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: ShittimChestConfig = snapshot.valid ? snapshot.config : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("shittimchest doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("shittimchest configure")}.`;
