...
import { discoverShittimChestPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `shittimchest-plugins-${randomUUID()}`);
  ...
  return await withEnvAsync(
    {
      ...
      SHITTIMCHEST_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
describe("discoverShittimChestPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".shittimchest", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverShittimChestPlugins({ workspaceDir });
    });
  ...
  it("ignores backup and disabled plugin directories in scanned roots", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverShittimChestPlugins({});
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        shittimchest: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        shittimchest: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverShittimChestPlugins({ extraPaths: [packDir] });
    });
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@shittimchest/pack",
        shittimchest: { extensions: ["./linked/escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it("rejects package extension entries that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@shittimchest/pack",
        shittimchest: { extensions: ["./escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@shittimchest/pack",
        shittimchest: { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverShittimChestPlugins({ ownershipUid: actualUid + 1 });
      });
      expect(result.candidates).toHaveLength(0);
