...
function createSinglePluginWithEmptySlotsConfig(): ShittimChestConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): ShittimChestConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "shittimchest-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-shittimchest-extensions/my-plugin",
      },
      extensionsDir,
    });
