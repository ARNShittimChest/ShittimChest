...
import { __testing, loadShittimChestPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `shittimchest-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.SHITTIMCHEST_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "shittimchest.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.SHITTIMCHEST_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadShittimChestPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadShittimChestPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.SHITTIMCHEST_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.SHITTIMCHEST_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadShittimChestPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadShittimChestPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadShittimChestPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.SHITTIMCHEST_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ SHITTIMCHEST_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
