...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("shittimchest-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("shittimchest-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("shittimchest-lid-a-", (first) => {
      withTempDirSync("shittimchest-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.shittimchest when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "shittimchest-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers SHITTIMCHEST_HOME over HOME", () => {
    vi.stubEnv("SHITTIMCHEST_HOME", "/srv/shittimchest-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/shittimchest-home"));
...
describe("shortenHomePath", () => {
  it("uses $SHITTIMCHEST_HOME prefix when SHITTIMCHEST_HOME is set", () => {
    vi.stubEnv("SHITTIMCHEST_HOME", "/srv/shittimchest-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $SHITTIMCHEST_HOME replacement when SHITTIMCHEST_HOME is set", () => {
    vi.stubEnv("SHITTIMCHEST_HOME", "/srv/shittimchest-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/shittimchest")).toBe(path.resolve(os.homedir(), "shittimchest"));
  });
  ...
  it("prefers SHITTIMCHEST_HOME for tilde expansion", () => {
    vi.stubEnv("SHITTIMCHEST_HOME", "/srv/shittimchest-home");
    vi.stubEnv("HOME", "/home/other");
