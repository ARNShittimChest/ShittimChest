...
export function resolveHooksConfig(cfg: ShittimChestConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ShittimChestConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-shittimchest-token"] === "string"
      ? req.headers["x-shittimchest-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
