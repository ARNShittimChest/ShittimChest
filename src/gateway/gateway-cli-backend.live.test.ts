...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("SHITTIMCHEST_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.SHITTIMCHEST_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `SHITTIMCHEST_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const baseCliArgs =
      parseJsonStringArray(
        "SHITTIMCHEST_LIVE_CLI_BACKEND_ARGS",
    ...
    const cliClearEnv =
      parseJsonStringArray(
        "SHITTIMCHEST_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.SHITTIMCHEST_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.SHITTIMCHEST_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.SHITTIMCHEST_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "SHITTIMCHEST_LIVE_CLI_BACKEND_IMAGE_MODE requires SHITTIMCHEST_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-live-cli-"));
    const disableMcpConfig = process.env.SHITTIMCHEST_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
