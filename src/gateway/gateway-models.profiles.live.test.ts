...
import { resolveShittimChestAgentDir } from "../agents/agent-paths.js";
...
import { ensureShittimChestModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, ShittimChestConfig, ModelProviderConfig } from "../config/types.js";
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.SHITTIMCHEST_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.SHITTIMCHEST_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: ShittimChestConfig;
  agentDir: string;
}): ShittimChestConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ShittimChestConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.SHITTIMCHEST_CONFIG_PATH,
    token: process.env.SHITTIMCHEST_GATEWAY_TOKEN,
    skipChannels: process.env.SHITTIMCHEST_SKIP_CHANNELS,
    skipGmail: process.env.SHITTIMCHEST_SKIP_GMAIL_WATCHER,
    skipCron: process.env.SHITTIMCHEST_SKIP_CRON,
    skipCanvas: process.env.SHITTIMCHEST_SKIP_CANVAS_HOST,
    agentDir: process.env.SHITTIMCHEST_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SHITTIMCHEST_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.SHITTIMCHEST_SKIP_CHANNELS = "1";
  process.env.SHITTIMCHEST_SKIP_GMAIL_WATCHER = "1";
  process.env.SHITTIMCHEST_SKIP_CRON = "1";
  process.env.SHITTIMCHEST_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.SHITTIMCHEST_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveShittimChestAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-live-state-"));
  process.env.SHITTIMCHEST_STATE_DIR = tempStateDir;
  ...
  process.env.SHITTIMCHEST_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.shittimchest-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveShittimChestAgentDir();
  const sanitizedCfg: ShittimChestConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-live-"));
  const tempConfigPath = path.join(tempDir, "shittimchest.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.SHITTIMCHEST_CONFIG_PATH = tempConfigPath;

  await ensureShittimChestModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "ShittimChest live tool probe (local, safe): " +
    ...
    process.env.SHITTIMCHEST_CONFIG_PATH = previous.configPath;
    process.env.SHITTIMCHEST_GATEWAY_TOKEN = previous.token;
    process.env.SHITTIMCHEST_SKIP_CHANNELS = previous.skipChannels;
    process.env.SHITTIMCHEST_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.SHITTIMCHEST_SKIP_CRON = previous.skipCron;
    process.env.SHITTIMCHEST_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.SHITTIMCHEST_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SHITTIMCHEST_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.SHITTIMCHEST_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via SHITTIMCHEST_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.shittimchest-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
