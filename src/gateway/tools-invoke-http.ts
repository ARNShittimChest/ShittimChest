...
import { createShittimChestTools } from "../agents/shittimchest-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-shittimchest-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-shittimchest-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-shittimchest-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-shittimchest-thread-id")?.trim() || undefined;
  ...
  const allTools = createShittimChestTools({
