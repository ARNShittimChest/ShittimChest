...
type GetReplyFromConfigFn = (
  ...
  configOverride?: ShittimChestConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `shittimchest-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SHITTIMCHEST_CONFIG_PATH = path.join(root, "shittimchest.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "shittimchest.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "shittimchest-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadShittimChestPlugins: () => pluginRegistryState.registry,
  };
});
