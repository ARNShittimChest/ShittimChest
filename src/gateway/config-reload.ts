...
export function resolveGatewayReloadSettings(cfg: ShittimChestConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: ShittimChestConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ShittimChestConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ShittimChestConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ShittimChestConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: ShittimChestConfig) => {
