...
let lastCreateShittimChestToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/shittimchest-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateShittimChestToolsContext?.agentTo,
          agentThreadId: lastCreateShittimChestToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createShittimChestTools: (ctx: Record<string, unknown>) => {
      lastCreateShittimChestToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateShittimChestToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-shittimchest-message-to": "channel:24514",
        "x-shittimchest-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
