...
const { createShittimChestTools } = await import("../agents/shittimchest-tools.js");
...
type SessionSendTool = ReturnType<typeof createShittimChestTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createShittimChestTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SHITTIMCHEST_GATEWAY_PORT", "SHITTIMCHEST_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.SHITTIMCHEST_GATEWAY_PORT = String(gatewayPort);
  process.env.SHITTIMCHEST_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("SHITTIMCHEST_CONFIG_PATH missing in gateway test environment");
      }
