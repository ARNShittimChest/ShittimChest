...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = ShittimChestSchema.safeParse({
      $schema: "https://shittimchest.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://shittimchest.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = ShittimChestSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = ShittimChestSchema.safeParse({
      cron: {
        enabled: true,
