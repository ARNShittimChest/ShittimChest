...
describe("config io write", () => {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".shittimchest", "shittimchest.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".shittimchest", "logs", "config-audit.jsonl");
  ...
  it("persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withTempHome("shittimchest-config-io-", async (home) => {
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withTempHome("shittimchest-config-io-", async (home) => {
      ...
      const invalidConfig: ShittimChestConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      } satisfies ShittimChestConfig;

      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "shittimchest config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'shittimchest config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withTempHome("shittimchest-config-io-", async (home) => {
      const configPath = path.join(home, ".shittimchest", "shittimchest.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withTempHome("shittimchest-config-io-", async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          SHITTIMCHEST_WATCH_MODE: "1",
          SHITTIMCHEST_WATCH_SESSION: "watch-session-1",
          SHITTIMCHEST_WATCH_COMMAND: "gateway --force",
