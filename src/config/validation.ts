...
import type { ShittimChestConfig, ConfigValidationIssue } from "./types.js";
import { ShittimChestSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: ShittimChestConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ShittimChestConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ShittimChestSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ShittimChestConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ShittimChestConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as ShittimChestConfig,
  };
}
