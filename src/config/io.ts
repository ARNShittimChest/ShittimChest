...
import type { ShittimChestConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareShittimChestVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  shittimchest config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  shittimchest config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: ShittimChestConfig,
  pathSegments: string[],
): { changed: boolean; next: ShittimChestConfig } {
...
function coerceConfig(value: unknown): ShittimChestConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ShittimChestConfig;
}
...
function stampConfigVersion(cfg: ShittimChestConfig): ShittimChestConfig {
...
function warnIfConfigFromFuture(
  ...
  const cmp = compareShittimChestVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ShittimChest (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ShittimChestConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ShittimChestConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ShittimChestConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ShittimChestConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ShittimChestConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SHITTIMCHEST_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SHITTIMCHEST_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.SHITTIMCHEST_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.SHITTIMCHEST_WATCH_SESSION === "string" &&
        deps.env.SHITTIMCHEST_WATCH_SESSION.trim().length > 0
          ? deps.env.SHITTIMCHEST_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.SHITTIMCHEST_WATCH_COMMAND === "string" &&
        deps.env.SHITTIMCHEST_WATCH_COMMAND.trim().length > 0
          ? deps.env.SHITTIMCHEST_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `SHITTIMCHEST_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: ShittimChestConfig | null = null;
let runtimeConfigSourceSnapshot: ShittimChestConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SHITTIMCHEST_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SHITTIMCHEST_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(
  config: ShittimChestConfig,
  sourceConfig?: ShittimChestConfig,
): void {
...
export function getRuntimeConfigSnapshot(): ShittimChestConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): ShittimChestConfig {
