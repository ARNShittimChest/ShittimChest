...
import type { ConfigFileSnapshot } from "./types.shittimchest.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.shittimchest/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/shittimchest/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/shittimchest/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/shittimchest/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/shittimchest/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(ShittimChestSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ShittimChestSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ShittimChestConfig";
    const hints = mapSensitivePaths(ShittimChestSchema, "", {});
