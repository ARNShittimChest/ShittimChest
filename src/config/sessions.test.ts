...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SHITTIMCHEST_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-shittimchest",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-shittimchest#general");
  });
  ...
  it("derives session transcripts dir from SHITTIMCHEST_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SHITTIMCHEST_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.shittimchest");
  ...
  it("resolves cross-agent paths when SHITTIMCHEST_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.shittimchest"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
      ...
      expect(sessionFile).toBe(
        path.join(
          path.resolve("/home/user/.shittimchest"),
          "agents",
          "bot1",
          "sessions",
          "sess-1.jsonl",
