...
function isWhatsAppConfigured(cfg: ShittimChestConfig): boolean {
...
function isGenericChannelConfigured(cfg: ShittimChestConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: ShittimChestConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ShittimChestConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: ShittimChestConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: ShittimChestConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ShittimChestConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: ShittimChestConfig, pluginId: string): ShittimChestConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
