...
describe("agent components", () => {
  const createCfg = (): ShittimChestConfig => ({}) as ShittimChestConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): ShittimChestConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/shittimchest-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/shittimchest",
    });
  });
});
