...
import type { ShittimChestConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredShittimChestTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "shittimchest.log"); // legacy single-file path

const LOG_PREFIX = "shittimchest";
...
function resolveSettings(): ResolvedSettings {
  let cfg: ShittimChestConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => ShittimChestConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.SHITTIMCHEST_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[shittimchest] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
