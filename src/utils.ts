...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".shittimchest");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SHITTIMCHEST_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SHITTIMCHEST_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SHITTIMCHEST_STATE_DIR.
