...
const defaultTools = createShittimChestCodingTools({ senderIsOwner: true });
...
describe("createShittimChestCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: ShittimChestConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createShittimChestCodingTools({
    ...
    const anthropicTools = createShittimChestCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: ShittimChestConfig = {
      tools: {
        exec: {
    ...
    const allowed = createShittimChestCodingTools({
    ...
    const denied = createShittimChestCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createShittimChestCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
