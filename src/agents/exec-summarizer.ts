/**
 * Summarize shell commands into human-readable labels for tool display.
 *
 * Recognizes common CLI tools (git, grep, npm, node, etc.) and produces
 * concise descriptions like "check git status" or "search files".
 */

import { asRecord } from "./tool-display-types.js";
import {
  binaryName,
  firstPositional,
  optionValue,
...
function summarizeKnownExec(words: string[]): string {
  ...
  if (bin === "shittimchest") {
    ...
    return sub ? `run shittimchest ${sub}` : "run shittimchest";
  }
