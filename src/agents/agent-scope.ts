...
type AgentEntry = NonNullable<NonNullable<ShittimChestConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: ShittimChestConfig): AgentEntry[] {
...
export function listAgentIds(cfg: ShittimChestConfig): string[] {
...
export function resolveDefaultAgentId(cfg: ShittimChestConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: ShittimChestConfig, agentId: string): AgentEntry | undefined {
...
export function resolveRunModelFallbacksOverride(params: {
  cfg: ShittimChestConfig | undefined;
  agentId?: string | null;
  sessionKey?: string | null;
}): string[] | undefined {
  if (!params.cfg) {
...
export function resolveAgentWorkspaceDir(cfg: ShittimChestConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: ShittimChestConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
