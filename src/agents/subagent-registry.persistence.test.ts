...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SHITTIMCHEST_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-subagent-"));
  ...
  it("uses isolated temp state when SHITTIMCHEST_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "shittimchest-test-state"));
  });
});
