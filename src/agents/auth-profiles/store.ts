 ...
 * A common mistake — caused by the similarity with the `shittimchest.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (shittimchest.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.SHITTIMCHEST_AUTH_STORE_READONLY === "1";
