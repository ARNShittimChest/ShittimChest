...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("shittimchest-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("shittimchest-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("shittimchest-sbx-");
