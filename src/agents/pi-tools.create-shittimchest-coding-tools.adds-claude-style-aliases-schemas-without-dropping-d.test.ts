...
describe("createShittimChestCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-read-"));
    try {
      ...
      const contents = "Hello from shittimchest read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createShittimChestCodingTools({ sandbox });
