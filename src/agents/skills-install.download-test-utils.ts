...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("shittimchest-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-skills-install-"));
  try {
