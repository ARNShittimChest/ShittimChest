...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.SHITTIMCHEST_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".shittimchest", `workspace-${profile}`);
  }
  return path.join(home, ".shittimchest", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".shittimchest";
