...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    shittimchest: process.env.SHITTIMCHEST_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.shittimchest === undefined) {
      delete process.env.SHITTIMCHEST_GATEWAY_TOKEN;
    } else {
      process.env.SHITTIMCHEST_GATEWAY_TOKEN = envSnapshot.shittimchest;
    }
  ...
  it("uses SHITTIMCHEST_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SHITTIMCHEST_GATEWAY_TOKEN = "local-env-token";
