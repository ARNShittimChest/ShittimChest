...
function asShittimChestConfig(config: Partial<ShittimChestConfig>): ShittimChestConfig {
  return config as ShittimChestConfig;
}

function createToolConfig() {
  return asShittimChestConfig({ agents: { list: [{ id: "main", default: true }] } });
}

function createMemoryGetToolOrThrow(config: ShittimChestConfig = createToolConfig()) {
...
describe("memory search citations", () => {
  it("appends source information when citations are enabled", async () => {
    ...
    const cfg = asShittimChestConfig({
      memory: { citations: "on" },
