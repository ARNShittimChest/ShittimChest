...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureShittimChestModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureShittimChestModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
  ...
  it("preserves non-empty agent apiKey/baseUrl for matching providers in merge mode", async () => {
    await withTempHome(async () => {
      ...
      await ensureShittimChestModelsJson({
        models: {
          mode: "merge",
          providers: {
            custom: {
  ...
  it("refreshes stale explicit moonshot model capabilities from implicit catalog", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        const modelPath = path.join(resolveShittimChestAgentDir(), "models.json");
