...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const shittimChestTmpDir = path.resolve(resolvePreferredShittimChestTmpDir());
  if (!isPathInside(shittimChestTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: shittimChestTmpDir });
  return resolved;
}
