...
import { resolvePreferredShittimChestTmpDir } from "../infra/tmp-shittimchest-dir.js";
...
async function withOutsideHardlinkInShittimChestTmp<T>(
  params: {
    shittimChestTmpDir: string;
  ...
  const symlinkPath = params.symlinkPrefix
    ...
    : undefined;
  try {
    if (isPathInside(params.shittimChestTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.shittimChestTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const shittimChestTmpDir = resolvePreferredShittimChestTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred ShittimChest tmp root",
      media: path.join(shittimChestTmpDir, "image.png"),
      expected: path.join(shittimChestTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred ShittimChest tmp root",
      media: pathToFileURL(path.join(shittimChestTmpDir, "photo.png")).href,
      expected: path.join(shittimChestTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred ShittimChest tmp root",
      media: path.join(shittimChestTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(shittimChestTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(shittimChestTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside shittimchest tmp root",
      media: path.join(os.tmpdir(), "outside-shittimchest", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked ShittimChest tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(shittimChestTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(shittimChestTmpDir, { recursive: true });
      const symlinkPath = path.join(shittimChestTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked ShittimChest tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInShittimChestTmp(
      {
        shittimChestTmpDir,
  ...
  it("rejects symlinked ShittimChest tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInShittimChestTmp(
