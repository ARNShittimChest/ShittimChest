...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createShittimChestTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-test-"));

    try {
      await withEnvAsync(
        { SHITTIMCHEST_STATE_DIR: stateDir, SHITTIMCHEST_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: shittimchest --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/shittimchest" } }\n}\n';
