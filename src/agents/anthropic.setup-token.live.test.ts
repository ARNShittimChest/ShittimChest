...
import { ensureShittimChestModelsJson } from "./models-config.js";
import { discoverAuthStorage, discoverModels } from "./pi-model-discovery.js";

const LIVE =
  isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.SHITTIMCHEST_LIVE_TEST);
const SETUP_TOKEN_RAW = process.env.SHITTIMCHEST_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.SHITTIMCHEST_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.SHITTIMCHEST_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const SETUP_TOKEN_MODEL = process.env.SHITTIMCHEST_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "SHITTIMCHEST_LIVE_SETUP_TOKEN did not look like a setup-token. Use SHITTIMCHEST_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set SHITTIMCHEST_LIVE_SETUP_TOKEN_VALUE or SHITTIMCHEST_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureShittimChestModelsJson(cfg, tokenSource.agentDir);
