    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "shittimchest.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "shittimchest.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("shittimchest-");
    const acpxRoot = await tempDirs.make("shittimchest-acpx-plugin-");
    const helperRoot = await tempDirs.make("shittimchest-helper-plugin-");
