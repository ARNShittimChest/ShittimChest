...
function resolveSkillsLimits(config?: ShittimChestConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "shittimchest-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "shittimchest-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveShittimChestMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: ShittimChestConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`shittimchest skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
