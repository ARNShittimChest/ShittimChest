...
const GOOGLE_LIVE_SINGLE_KEY = "SHITTIMCHEST_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SHITTIMCHEST_LIVE_ANTHROPIC_KEY",
    listVar: "SHITTIMCHEST_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SHITTIMCHEST_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SHITTIMCHEST_LIVE_${base}_KEY`;
