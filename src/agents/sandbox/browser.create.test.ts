...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    workspaceRoot: "/tmp/shittimchest-sandboxes",
    docker: {
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("SHITTIMCHEST_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("SHITTIMCHEST_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^SHITTIMCHEST_BROWSER_NOVNC_PASSWORD=[a-f0-9]{8}$/);
