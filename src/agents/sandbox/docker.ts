...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "shittimchest.sandbox=1");
  args.push("--label", `shittimchest.sessionKey=${params.scopeKey}`);
  args.push("--label", `shittimchest.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `shittimchest.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "shittimchest.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`shittimchest sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`shittimchest sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("shittimchest sandbox recreate --all");
}
