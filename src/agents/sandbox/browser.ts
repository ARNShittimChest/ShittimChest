...
import {
  ...
  DEFAULT_SHITTIMCHEST_BROWSER_COLOR,
  DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME,
...
const CDP_SOURCE_RANGE_ENV_KEY = "SHITTIMCHEST_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_SHITTIMCHEST_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "shittimchest.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `shittimchest sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `shittimchest sandbox recreate --browser --agent ${agentId}`;
          }
          return "shittimchest sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        ...
        "shittimchest.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `SHITTIMCHEST_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SHITTIMCHEST_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "SHITTIMCHEST_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_SHITTIMCHEST_BROWSER_PROFILE_NAME)
    : null;
