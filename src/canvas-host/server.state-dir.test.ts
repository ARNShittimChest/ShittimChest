...
describe("canvas host state dir defaults", () => {
  it("uses SHITTIMCHEST_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("shittimchest-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("ShittimChest Canvas");
      } finally {
        await handler.close();
      }
    });
