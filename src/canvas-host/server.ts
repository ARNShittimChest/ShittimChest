...
<title>ShittimChest Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.shittimchestCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.shittimchestCanvasA2UIAction &&
        typeof window.shittimchestCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.shittimchestSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("shittimchest:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ShittimChest node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.shittimchestSendUserAction === "function"
        ? window.shittimchestSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SHITTIMCHEST_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SHITTIMCHEST_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ShittimChest Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
