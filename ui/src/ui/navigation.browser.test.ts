...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/shittimchest/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/shittimchest");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/shittimchest/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SHITTIMCHEST_CONTROL_UI_BASE_PATH__ = "/shittimchest";
    const app = mountApp("/shittimchest/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/shittimchest");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/shittimchest/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "shittimchest.control.settings.v1",
