...
export async function handleWhatsAppStart(host: ShittimChestApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ShittimChestApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ShittimChestApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ShittimChestApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ShittimChestApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ShittimChestApp): string {
...
function resolveGatewayHttpAuthHeader(host: ShittimChestApp): string | null {
...
function buildGatewayHttpHeaders(host: ShittimChestApp): Record<string, string> {
  const authorization = resolveGatewayHttpAuthHeader(host);
  return authorization ? { Authorization: authorization } : {};
}

export function handleNostrProfileEdit(
  host: ShittimChestApp,
  accountId: string,
...
export function handleNostrProfileCancel(host: ShittimChestApp) {
...
export function handleNostrProfileToggleAdvanced(host: ShittimChestApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ShittimChestApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ShittimChestApp) {
  const state = host.nostrProfileFormState;
