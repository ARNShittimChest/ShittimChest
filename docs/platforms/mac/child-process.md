...
service via the external `shittimchest` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.shittimchest.gateway`
  (or `ai.shittimchest.<profile>` when using `--profile`/`SHITTIMCHEST_PROFILE`; legacy `com.shittimchest.*` is supported).
