...
- Same path: run the app from a fixed location (for ShittimChest, `dist/ShittimChest.app`).
...
```bash
sudo tccutil reset Accessibility ai.shittimchest.mac
sudo tccutil reset ScreenCapture ai.shittimchest.mac
...
Workaround: move files into the ShittimChest workspace (`~/.shittimchest/workspace`) if you want to avoid per-folder grants.
