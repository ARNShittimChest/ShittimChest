---
summary: "ShittimChest on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ShittimChest on a Raspberry Pi
  - Running ShittimChest on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ShittimChest on Raspberry Pi

## Goal

Run a persistent, always-on ShittimChest Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most ShittimChest features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled shittimchest

# Enable if not
sudo systemctl enable shittimchest

# Start on boot
sudo systemctl start shittimchest
```
...
```bash
# Check logs
journalctl -u shittimchest --no-pager -n 100

# Common fix: rebuild
cd ~/shittimchest  # if using hackable install
npm run build
sudo systemctl restart shittimchest
```
