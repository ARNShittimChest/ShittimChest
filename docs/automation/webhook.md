...
read_when:
  ...
  - Wiring external systems into ShittimChest
title: "Webhooks"
---
...
```json5
{
  hooks: {
    enabled: true,
    token: "${SHITTIMCHEST_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your ShittimChest config directory (typically `~/.shittimchest/hooks/transforms`).
...
- `shittimchest webhooks gmail setup` writes `hooks.gmail` config for `shittimchest webhooks gmail run`.
