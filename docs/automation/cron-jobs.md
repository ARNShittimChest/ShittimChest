...
- Jobs persist under `~/.shittimchest/cron/` so restarts don’t lose schedules.
...
shittimchest cron run <job-id>
shittimchest cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.shittimchest/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `shittimchest cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, ShittimChest applies a
...
If `delivery` is omitted for isolated jobs, ShittimChest defaults to `announce`.
...
- Job store: `~/.shittimchest/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.shittimchest/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, ShittimChest classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.shittimchest/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, ShittimChest archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `shittimchest cron runs` and adjust retention before logs become large
...
shittimchest cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
shittimchest cron edit <jobId> --agent ops
shittimchest cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
shittimchest cron run <jobId>
shittimchest cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
shittimchest cron edit <jobId> \
...
```bash
shittimchest cron edit <jobId> --exact
```
...
```bash
shittimchest system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`shittimchest system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SHITTIMCHEST_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- ShittimChest applies exponential retry backoff for recurring jobs after consecutive errors:
