...
- `/usage cost` in chats: local cost summary aggregated from ShittimChest session logs.
- CLI: `shittimchest status --usage` prints a full per-provider breakdown.
- CLI: `shittimchest channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
