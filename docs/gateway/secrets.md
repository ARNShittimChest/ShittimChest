...
ShittimChest supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, ShittimChest performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). ShittimChest validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/ShittimChest QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/shittimchest"],
...
When reload-time activation fails after a healthy state, ShittimChest enters degraded secrets state.
...
```bash
shittimchest secrets audit --check
shittimchest secrets configure
shittimchest secrets audit --check
```
...
- plaintext values at rest (`shittimchest.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `shittimchest.json`
...
- `shittimchest secrets configure --providers-only`
- `shittimchest secrets configure --skip-provider-setup`
...
```bash
shittimchest secrets apply --from /tmp/shittimchest-secrets-plan.json
shittimchest secrets apply --from /tmp/shittimchest-secrets-plan.json --dry-run
```
...
ShittimChest intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, ShittimChest runtime no longer depends on plaintext `auth.json`.
