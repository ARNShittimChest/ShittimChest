...
> ShittimChest is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
ShittimChest security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `shittimchest security audit`
...
```bash
shittimchest security audit
shittimchest security audit --deep
shittimchest security audit --fix
shittimchest security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

ShittimChest is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
ShittimChest assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.shittimchest`, including `shittimchest.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- ShittimChest can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want ShittimChest, use one VPS/host per user.
...
ShittimChest is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/shittimchest/shittimchest/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, ShittimChest also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.shittimchest/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.shittimchest/credentials/<channel>-allowFrom.json` (default account)
  - `~/.shittimchest/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.shittimchest/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.shittimchest/secrets.json`
- **Legacy OAuth import**: `~/.shittimchest/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full ShittimChest state                           | filesystem perms on `~/.shittimchest`                                                             | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.shittimchest/shittimchest.json`                                           | yes      |
...
`shittimchest security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`shittimchest security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in ShittimChest config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${SHITTIMCHEST_GATEWAY_PASSWORD}
```
...
- ShittimChest gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from ShittimChest responses.
...
ShittimChest stores session transcripts on disk under `~/.shittimchest/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.shittimchest` (see the audit section below). If you need
...
ShittimChest can refresh the skills list mid-session:
...
- If you install plugins from npm (`shittimchest plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.shittimchest/extensions/<pluginId>/` (or `$SHITTIMCHEST_STATE_DIR/extensions/<pluginId>/`).
  - ShittimChest uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
shittimchest pairing list <channel>
shittimchest pairing approve <channel> <code>
```
...
By default, ShittimChest routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ShittimChest has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.shittimchest/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.shittimchest or your logs.”

## Unsafe external content bypass flags

ShittimChest includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.shittimchest/shittimchest.json`: `600` (user read/write only)
- `~/.shittimchest`: `700` (user only)

`shittimchest doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `SHITTIMCHEST_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__shittimchest__/canvas/` and `/__shittimchest__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_shittimchest-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `SHITTIMCHEST_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `shittimchest doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `SHITTIMCHEST_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `SHITTIMCHEST_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ShittimChest
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. ShittimChest verifies the identity by resolving the
...
- ShittimChest will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.shittimchest/` (or `$SHITTIMCHEST_STATE_DIR/`) may contain secrets or private data:

- `shittimchest.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `shittimchest status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@shittimchest", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.shittimchest`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.shittimchest/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `shittimchest` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only ShittimChest clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

ShittimChest’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default ShittimChest limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `shittimchest gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `SHITTIMCHEST_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/shittimchest/shittimchest-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.shittimchest/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `shittimchest security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + ShittimChest version
...
Found a vulnerability in ShittimChest? Please report responsibly:

1. Email: [security@shittimchest.ai](mailto:security@shittimchest.ai)
