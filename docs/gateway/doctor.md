...
`shittimchest doctor` is the repair + migration tool for ShittimChest. It fixes stale
...
```bash
shittimchest doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
shittimchest doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
shittimchest doctor --repair --force
```
...
```bash
cat ~/.shittimchest/shittimchest.json
```
...
- Extra workspace dir detection (`~/shittimchest`).
...
you to run `shittimchest doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.shittimchest/shittimchest.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.shittimchest/sessions/` to `~/.shittimchest/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.shittimchest/agent/` to `~/.shittimchest/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.shittimchest/credentials/*.json` (except `oauth.json`)
  - to `~/.shittimchest/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `shittimchest doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.shittimchest` folders exist across
  home directories or when `SHITTIMCHEST_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.shittimchest/shittimchest.json` is
...
offers to remove them and install the ShittimChest service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named ShittimChest gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `shittimchest doctor --generate-gateway-token` to force token
...
- `shittimchest doctor` prompts before rewriting supervisor config.
- `shittimchest doctor --yes` accepts the default repair prompts.
- `shittimchest doctor --repair` applies recommended fixes without prompts.
- `shittimchest doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `shittimchest gateway install --force`.
