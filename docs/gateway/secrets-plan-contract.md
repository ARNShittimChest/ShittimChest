...
read_when:
  - Generating or reviewing `shittimchest secrets apply` plan files
...
This page defines the strict contract enforced by `shittimchest secrets apply`.

If a target does not match these rules, apply fails before mutating config.

## Plan file shape

`shittimchest secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
If apply fails with an invalid target path message, regenerate the plan with `shittimchest secrets configure` or fix the target path to one of the allowed shapes above.
