...
- During heartbeat runs, ShittimChest treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `shittimchest sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ShittimChest skips the heartbeat run entirely (no model call).
...
```bash
shittimchest system event --text "Check for urgent follow-ups" --mode now
```
