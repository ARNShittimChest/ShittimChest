...
read_when:
  - Running ShittimChest behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of ShittimChest
...
- You run ShittimChest behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. ShittimChest checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. ShittimChest extracts the user identity from the configured header
...
- ShittimChest can stay on loopback HTTP behind the proxy.
...
If ShittimChest itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://shittimchest.example.com
    to: http://shittimchest-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
shittimchest.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy shittimchest:18789 {
...
location / {
    ...
    proxy_pass http://shittimchest:18789;
...
`shittimchest security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update ShittimChest config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `shittimchest security audit` and review findings
