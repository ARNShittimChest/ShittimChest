---
summary: "CLI reference for `shittimchest secrets` (reload, audit, configure, apply)"
read_when:
...
# `shittimchest secrets`

Use `shittimchest secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
```bash
shittimchest secrets reload
shittimchest secrets reload --json
```
...
```bash
shittimchest secrets audit
shittimchest secrets audit --check
shittimchest secrets audit --json
```
...
```bash
shittimchest secrets configure
shittimchest secrets configure --plan-out /tmp/shittimchest-secrets-plan.json
shittimchest secrets configure --apply --yes
shittimchest secrets configure --providers-only
shittimchest secrets configure --skip-provider-setup
shittimchest secrets configure --json
```
...
- `configure` targets secret-bearing fields in `shittimchest.json`.
...
shittimchest secrets apply --from /tmp/shittimchest-secrets-plan.json
shittimchest secrets apply --from /tmp/shittimchest-secrets-plan.json --dry-run
shittimchest secrets apply --from /tmp/shittimchest-secrets-plan.json --json
```
...
- `shittimchest.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.shittimchest/.env` known secret keys whose values were migrated
