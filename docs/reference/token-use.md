---
summary: "How ShittimChest builds prompt context and reports token usage + costs"
read_when:
...
ShittimChest tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

ShittimChest assembles its own system prompt on every run. It includes:
...
For images, ShittimChest downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from ShittimChest session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `shittimchest status --usage` and `shittimchest channels list` show
...
`cacheWrite`. If pricing is missing, ShittimChest shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. ShittimChest can
...
Anthropic's 1M context window is currently beta-gated. ShittimChest can inject the
...
ShittimChest skips the `context-1m-*` beta header because Anthropic currently
