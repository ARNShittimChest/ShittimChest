...
- With Anthropic API-key auth profiles, ShittimChest seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, ShittimChest injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
ShittimChest exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.shittimchest/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SHITTIMCHEST_STATE_DIR/logs/cache-trace.jsonl`
...
- `SHITTIMCHEST_CACHE_TRACE=1` enables cache tracing.
- `SHITTIMCHEST_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SHITTIMCHEST_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SHITTIMCHEST_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SHITTIMCHEST_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
