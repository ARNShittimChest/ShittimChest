...
ShittimChest features that can generate provider usage or paid API calls.
...
- `shittimchest status --usage` and `shittimchest channels list` show provider **usage windows**
...
ShittimChest can pick up credentials from:
...
- `shittimchest models status --json`
...
`shittimchest models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
