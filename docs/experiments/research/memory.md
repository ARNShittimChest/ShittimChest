...
read_when:
  - Designing workspace memory (~/.shittimchest/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ShittimChest integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.shittimchest/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.shittimchest/workspace` as canonical human-readable memory.
...
```
~/.shittimchest/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ShittimChest**, but keep a separable core library.

### Why integrate into ShittimChest?

- ShittimChest already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `shittimchest memory recall "…" --k 25 --since 30d`
  - `shittimchest memory reflect --since 7d`
...
Pragmatic take for `~/.shittimchest/workspace`:
