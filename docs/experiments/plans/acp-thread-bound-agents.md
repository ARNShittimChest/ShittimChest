...
This plan defines how ShittimChest should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- ShittimChest core owns ACP control plane concerns
...
ShittimChest should not reimplement ACP transport internals in core.
ShittimChest should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in ShittimChest, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under ShittimChest state dir
...
Core (must be in ShittimChest core):
...
- ACP event stream is normalized to ShittimChest reply chunks
...
For the first production backend (`extensions/acpx`), ShittimChest and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses ShittimChest `sessionKey`)
...
- ShittimChest consumes ndjson events from `acpx --format json --json-strict`
