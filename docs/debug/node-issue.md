...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ShittimChest
...
Running ShittimChest via Node with `tsx` fails at startup with:

```
[shittimchest] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `shittimchest status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch shittimchest.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node shittimchest.mjs status` works on Node 25.
