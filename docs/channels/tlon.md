...
Tlon is a decentralized messenger built on Urbit. ShittimChest connects to your Urbit ship and can
...
```bash
shittimchest plugins install @shittimchest/tlon
```

Local checkout (when running from a git repo):

```bash
shittimchest plugins install ./extensions/tlon
```
...
By default, ShittimChest blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `shittimchest message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ShittimChest replies in-thread.
