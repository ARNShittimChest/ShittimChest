...
```
    ...
    Telegram does **not** use `shittimchest channels login telegram`; configure token in config/env, then start gateway.
...
shittimchest pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `shittimchest doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `shittimchest doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
    ...
    1. DM your bot.
    2. Run `shittimchest logs --follow`.
    3. Read `from.id`.
...
```
    ...
    - or read `chat.id` from `shittimchest logs --follow`
...
- DM messages can carry `message_thread_id`; ShittimChest routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ShittimChest can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, ShittimChest keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), ShittimChest falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, ShittimChest skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, ShittimChest retries as plain text.
...
```
    ...
    - `~/.shittimchest/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, ShittimChest enqueues system events like:
...
```bash
shittimchest message send --channel telegram --target 123456789 --message "hi"
shittimchest message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `shittimchest channels status` warns when config expects unmentioned group messages.
    - `shittimchest channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `shittimchest logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, ShittimChest now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `SHITTIMCHEST_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `SHITTIMCHEST_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `SHITTIMCHEST_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `shittimchest doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `shittimchest doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
