---
summary: "Mattermost bot setup and ShittimChest config"
read_when:
...
```bash
shittimchest plugins install @shittimchest/mattermost
```

Local checkout (when running from a git repo):

```bash
shittimchest plugins install ./extensions/mattermost
```
...
4. Configure ShittimChest and start the gateway.
...
- Approve via:
  - `shittimchest pairing list mattermost`
  - `shittimchest pairing approve mattermost <CODE>`
...
Use these target formats with `shittimchest message send` or cron/webhooks:
