...
read_when:
  - Setting up Twitch chat integration for ShittimChest
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ShittimChest connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
shittimchest plugins install @shittimchest/twitch
```

Local checkout (when running from a git repo):

```bash
shittimchest plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SHITTIMCHEST_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "shittimchest", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SHITTIMCHEST_TWITCH_ACCESS_TOKEN env var)
...
```bash
SHITTIMCHEST_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
