...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ShittimChest to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
shittimchest plugins install @shittimchest/feishu
```

Local checkout (when running from a git repo):

```bash
shittimchest plugins install ./extensions/feishu
```
...
If you just installed ShittimChest, run the wizard:
...
2. Configuring app credentials in ShittimChest
...
1. You already ran `shittimchest channels add` for Feishu
2. The gateway is running (`shittimchest gateway status`)
...
Edit `~/.shittimchest/shittimchest.json`:
...
```bash
shittimchest pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  shittimchest pairing list feishu
  shittimchest pairing approve feishu <CODE>
  ```
...
2. Run `shittimchest logs --follow` and look for `chat_id`
...
2. Run `shittimchest logs --follow` and look for `open_id`
...
| `shittimchest gateway restart` | Restart gateway service       |
| `shittimchest logs --follow`   | Tail gateway logs             |
...
4. Check logs: `shittimchest logs --follow`
...
5. Ensure the gateway is running: `shittimchest gateway status`
6. Check logs: `shittimchest logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.shittimchest/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.shittimchest/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
