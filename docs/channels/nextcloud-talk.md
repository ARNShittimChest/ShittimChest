...
```bash
shittimchest plugins install @shittimchest/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
shittimchest plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ShittimChest" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `shittimchest pairing list nextcloud-talk`
  - `shittimchest pairing approve nextcloud-talk <CODE>`
