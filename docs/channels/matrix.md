...
Matrix is an open, decentralized messaging protocol. ShittimChest connects as a Matrix **user**
...
```bash
shittimchest plugins install @shittimchest/matrix
```

Local checkout (when running from a git repo):

```bash
shittimchest plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `shittimchest plugins install @shittimchest/matrix`
   - From a local checkout: `shittimchest plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ShittimChest calls the same
     login endpoint, stores the access token in `~/.shittimchest/credentials/matrix/credentials.json`,
...
- On first connection, ShittimChest requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ShittimChest logs a warning.
...
`~/.shittimchest/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `shittimchest pairing list matrix`
  - `shittimchest pairing approve matrix <CODE>`
...
- On startup, ShittimChest resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
