...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/shittimchest`)
...
If the page fails to load, start the Gateway first: `shittimchest gateway`.
...
you revoke it with `shittimchest devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.shittimchest/shittimchest.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop shittimchest`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ShittimChest
...
ShittimChest **blocks** Control UI connections without device identity.
