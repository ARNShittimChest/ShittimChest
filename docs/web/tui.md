...
```bash
shittimchest tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `shittimchest tui --deliver`
...
- Check the Gateway logs: `shittimchest logs --follow`.
- Confirm the agent can run: `shittimchest status` and `shittimchest models status`.
...
- No agents in picker: check `shittimchest agents list` and your routing config.
