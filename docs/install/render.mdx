...
Deploy ShittimChest on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/shittimchest/shittimchest)
...
repository configures everything needed to run ShittimChest:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SHITTIMCHEST_STATE_DIR
        value: /data/.shittimchest
      - key: SHITTIMCHEST_WORKSPACE_DIR
        value: /data/workspace
      - key: SHITTIMCHEST_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: shittimchest-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/shittimchest`.
...
If you use the original ShittimChest repository, Render will not auto-deploy your ShittimChest. To update it, run a manual Blueprint sync from the dashboard.
...
For ShittimChest, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ShittimChest host.
