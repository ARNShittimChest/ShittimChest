...
- `~/Library/Application Support/ShittimChest/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `shittimchest-canvas://<session>/<path>`

示例：

- `shittimchest-canvas://main/` → `<canvasRoot>/main/index.html`
- `shittimchest-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `shittimchest-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
shittimchest nodes canvas present --node <id>
shittimchest nodes canvas navigate --node <id> --url "/"
shittimchest nodes canvas eval --node <id> --js "document.title"
shittimchest nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__shittimchest__/a2ui/
```
...
shittimchest nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
shittimchest nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "shittimchest://agent?message=Review%20this%20design";
```
