...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`shittimchest setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.shittimchest/shittimchest.json` 和智能体工作区。
...
```bash
shittimchest setup
shittimchest setup --workspace ~/.shittimchest/workspace
```

通过 setup 运行向导：

```bash
shittimchest setup --wizard
```
