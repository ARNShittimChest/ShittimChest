...
- Plugin manifests (`shittimchest.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`shittimchest.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `shittimchest doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `shittimchest doctor --fix`.”
