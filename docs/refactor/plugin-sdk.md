...
- Publish as `shittimchest/plugin-sdk` (or export from core under `shittimchest/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ShittimChestPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ShittimChestConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ShittimChestConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ShittimChestConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ShittimChestConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ShittimChestConfig): string;
  };
};
```
...
- Introduce `shittimchest/plugin-sdk`.
- Add `api.runtime` to `ShittimChestPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `shittimchestRuntime: ">=2026.2.0"`).
