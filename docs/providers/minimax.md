---
summary: "Use MiniMax M2.1 in ShittimChest"
read_when:
  - You want MiniMax models in ShittimChest
...
```bash
shittimchest plugins enable minimax-portal-auth  # skip if already loaded.
shittimchest gateway restart  # restart if gateway is already running
shittimchest onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/shittimchest/shittimchest/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `shittimchest.json`:
...
- Use `shittimchest models list` and `shittimchest models set minimax/MiniMax-M2.1` to switch.
...
- Running `shittimchest configure` and selecting **MiniMax M2.1**, or
