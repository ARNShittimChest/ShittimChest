---
summary: "Use Venice AI privacy-focused models in ShittimChest"
read_when:
  - You want privacy-focused inference in ShittimChest
...
```bash
shittimchest onboard --auth-choice venice-api-key
```
...
```bash
shittimchest agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, ShittimChest shows all available Venice models. Pick based on your needs:
...
```bash
shittimchest models set venice/claude-opus-45
shittimchest models set venice/llama-3.3-70b
```

List all available models:

```bash
shittimchest models list | grep venice
```

## Configure via `shittimchest configure`

1. Run `shittimchest configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ShittimChest automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
shittimchest agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
shittimchest agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
shittimchest agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
shittimchest agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
shittimchest agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `shittimchest models list` to see currently available models. Some models may be temporarily offline.
