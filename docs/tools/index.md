---
summary: "Agent tool surface for ShittimChest (browser, canvas, nodes, message, cron) replacing legacy `shittimchest-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `shittimchest-*` skills
title: "Tools"
---

# Tools (ShittimChest)

ShittimChest exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `shittimchest-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `shittimchest.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ShittimChest logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:shittimchest`: all built-in ShittimChest tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`shittimchest node run`).
...
ShittimChest tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `shittimchest configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ShittimChest-managed browser.
...
- Quick smoke: `shittimchest nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `shittimchest gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, ShittimChest uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, ShittimChest clamps `tools.sessions.visibility` to `tree`.
