...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `shittimchest sandbox explain`.
...
Legacy `agent.*` configs are migrated by `shittimchest doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=shittimchest-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SHITTIMCHEST_STATE_DIR:-$HOME/.shittimchest}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
