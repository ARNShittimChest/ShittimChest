---
summary: "Chrome extension: let ShittimChest drive your existing Chrome tab"
read_when:
...
The ShittimChest Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate shittimchest-managed Chrome profile.
...
ShittimChest then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ShittimChest release (npm package) as static files. There is no separate “build” step.

After upgrading ShittimChest:

- Re-run `shittimchest browser extension install` to refresh the installed files under your ShittimChest state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

ShittimChest ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `SHITTIMCHEST_GATEWAY_TOKEN`)
...
- Open the tab you want ShittimChest to control.
...
- `ON`: attached; ShittimChest can drive that tab.
...
Debugging: `shittimchest sandbox explain`
...
`shittimchest browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `shittimchest browser extension install` first to copy the extension to a stable location under your ShittimChest state directory.
...
- **This is not isolated** like the dedicated shittimchest-managed profile.
