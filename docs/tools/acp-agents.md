...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let ShittimChest run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask ShittimChest in plain language to "run this in Codex" or "start Claude Code in a thread", ShittimChest should route that request to the ACP runtime (not the native sub-agent runtime).
...
- ShittimChest binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, ShittimChest returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, ShittimChest uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, ShittimChest may default to persistent behavior per runtime path
...
If no target resolves, ShittimChest returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, ShittimChest returns a clear unsupported-control error.
...
When ShittimChest uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal ShittimChest `agentId` path).
...
```bash
shittimchest plugins install @shittimchest/acpx
shittimchest config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
shittimchest plugins install ./extensions/acpx
```
...
`@shittimchest/acpx` now enforces a strict plugin-local pinning model:
...
- ShittimChest startup stays non-blocking while acpx ensure runs.
