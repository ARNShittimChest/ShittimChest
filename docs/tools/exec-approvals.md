...
- **gateway host** → `shittimchest` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.shittimchest/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.shittimchest/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `shittimchest approvals allowlist ...`).
- `shittimchest security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `shittimchest doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.shittimchest/exec-approvals.json` directly.

CLI: `shittimchest approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
