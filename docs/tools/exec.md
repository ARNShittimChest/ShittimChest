...
- `gateway`/`node` approvals are controlled by `~/.shittimchest/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  ShittimChest prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`shittimchest security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `shittimchest doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
