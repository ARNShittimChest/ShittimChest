import type { AnyAgentTool, ShittimChestPluginApi } from "shittimchest/plugin-sdk";
...
export function createToolFactoryHarness(cfg: ShittimChestPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<ShittimChestPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as ShittimChestPluginApi,
    resolveTool,
  };
}
