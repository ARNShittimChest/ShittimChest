/**
 * Twitch channel plugin for ShittimChest.
 *
...
import { buildChannelConfigSchema } from "shittimchest/plugin-sdk";
 ...
 * for ShittimChest. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ShittimChestConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ShittimChestConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ShittimChestConfig): boolean => {
