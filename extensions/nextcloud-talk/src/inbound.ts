...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as ShittimChestConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as ShittimChestConfig,
    surface: CHANNEL_ID,
  });
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ShittimChestConfig);
