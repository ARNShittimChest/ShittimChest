...
ShittimChest extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
shittimchest channels login --channel zalouser
...
```bash
shittimchest message send --channel zalouser --target <threadId> --message "Hello from ShittimChest!"
```
...
```bash
shittimchest channels login --channel zalouser              # Login via QR
shittimchest channels login --channel zalouser --account work
shittimchest channels status --probe
shittimchest channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
shittimchest directory self --channel zalouser
shittimchest directory peers list --channel zalouser --query "name"
shittimchest directory groups list --channel zalouser --query "work"
shittimchest directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
shittimchest message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
shittimchest message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `shittimchest channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
shittimchest channels login --channel zalouser --account work
shittimchest message send --channel zalouser --account work --target <id> --message "Hello"
