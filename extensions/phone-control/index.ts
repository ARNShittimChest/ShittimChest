...
import type { ShittimChestPluginApi, ShittimChestPluginService } from "shittimchest/plugin-sdk";
...
function normalizeDenyList(cfg: ShittimChestPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ShittimChestPluginApi["config"]): string[] {
...
export default function register(api: ShittimChestPluginApi) {
  ...
  const timerService: ShittimChestPluginService = {
