...
metadata: { "shittimchest": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ShittimChest `sessions_spawn`
- **File I/O** == ShittimChest `read`/`write`
- **Remote fetch** == ShittimChest `web_fetch` (or `exec` with curl when POST is required)
