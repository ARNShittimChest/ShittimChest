...
function resolveShittimChestRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SHITTIMCHEST_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["shittimchest"]) {
  ...
  throw new Error("Unable to resolve core root. Set SHITTIMCHEST_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveShittimChestRoot(), "dist", "extensionAPI.js");
