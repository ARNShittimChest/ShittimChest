# Google Gemini CLI Auth (ShittimChest plugin)
...
```bash
shittimchest plugins enable google-gemini-cli-auth
```
...
```bash
shittimchest models auth login --provider google-gemini-cli --set-default
```
...
- `SHITTIMCHEST_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `SHITTIMCHEST_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
