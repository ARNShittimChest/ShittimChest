...
`shittimchest.invoke --each`) without adding new ShittimChest code per workflow.
...
This extension depends on ShittimChest internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ShittimChest extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.shittimchest/extensions` as a standalone plugin directory.
