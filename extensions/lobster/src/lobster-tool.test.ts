...
function fakeApi(overrides: Partial<ShittimChestPluginApi> = {}): ShittimChestPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "shittimchest-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ShittimChestPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
