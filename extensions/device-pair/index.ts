...
function resolveGatewayPort(cfg: ShittimChestPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.SHITTIMCHEST_GATEWAY_PORT?.trim()) ??
...
function resolveScheme(
  cfg: ShittimChestPluginApi["config"],
...
function resolveAuth(cfg: ShittimChestPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: ShittimChestPluginApi): Promise<ResolveUrlResult> {
...
export default function register(api: ShittimChestPluginApi) {
  api.registerCommand({
    name: "pair",
