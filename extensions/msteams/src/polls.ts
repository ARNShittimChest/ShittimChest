...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["shittimchestPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["shittimchest", "pollId"]) ??
    readNestedString(value, ["shittimchest", "poll", "id"]) ??
    readNestedString(value, ["data", "shittimchestPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "shittimchest", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          shittimchestPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "shittimchest poll vote",
          displayText: "Vote recorded",
          value: { shittimchestPollId: pollId, pollId },
        },
      },
    ],
  };
